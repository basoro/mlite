...
import { ClipboardList, Calendar, Clock, User, ChevronRight, Bed, Filter, X } from 'lucide-react-native';
...
export default function RawatInapScreen() {
  const [data, setData] = useState<RawatInap[]>([]);
  ...
  const [status, setStatus] = useState(''); // 'Masih Rawat', 'Pulang', etc.
  ...
  const fetchData = async (isRefresh = false) => {
    try {
      ...
      const response = await api.rawatInap.list({ 
        page: 1, 
        per_page: 20,
        tgl_awal: '2025-01-01', // Temporary hardcoded for testing
        tgl_akhir: '2025-12-31', // Temporary hardcoded for testing
        // tgl_awal: startDate,
        // tgl_akhir: endDate,
        // stts_pulang: status || undefined
      });
      // Adjust based on actual API response structure
      console.log('API Response Rawat Inap:', response.data);
  ...
  const renderItem = ({ item }: { item: RawatInap }) => (
    <TouchableOpacity 
      style={styles.card}
      onPress={() => {
        ...
        router.push(`/detail/rawat-inap/${normalizedNoRawat}`);
      }}
    >
      <View style={styles.cardHeader}>
        ...
        <View style={styles.badge}>
          <Text style={styles.badgeText}>{item.lama} Hari</Text>
        </View>
      </View>
      ...
      <View style={styles.cardBody}>
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoText}>{item.tgl_masuk}</Text>
          <Clock size={14} color="#666" style={{ marginLeft: 8 }} />
          <Text style={styles.infoText}>{item.jam_masuk}</Text>
        </View>

        <View style={styles.infoRow}>
          <Bed size={14} color="#666" />
          <Text style={styles.infoText}>{item.nm_kamar}</Text>
        </View>
    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.headerTop}>
          <View>
            <Text style={styles.headerTitle}>Rawat Inap</Text>
            <Text style={styles.headerSubtitle}>Kelola pasien rawat inap</Text>
          </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              <ClipboardList color="#CCC" size={48} />
              <Text style={styles.emptyText}>Tidak ada data pasien rawat inap</Text>
            </View>
          }
        />
      )}
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.formGroup}>
              <Text style={styles.label}>Status Pulang</Text>
              <View style={styles.statusContainer}>
                ...
                <TouchableOpacity 
                  ...
                  onPress={() => setStatus('-')}
                >
                  <Text style={[styles.statusText, status === '-' && styles.statusTextActive]}>Belum</Text>
                </TouchableOpacity>
                <TouchableOpacity 
                  style={[styles.statusButton, status === 'Pulang' && styles.statusButtonActive]}
                  onPress={() => setStatus('Pulang')}
                >
                  <Text style={[styles.statusText, status === 'Pulang' && styles.statusTextActive]}>Pulang</Text>
                </TouchableOpacity>
              </View>
            </View>
