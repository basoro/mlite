...
$(document).ready(function(){
    ...
    function load_data(keyword) {
      $.ajax({
        type: 'GET',
        url: 'http://localhost/Khanza-Lite/admin/pendaftaran/ajax?keyword='+keyword+'&t=05478e6e8c72',
...
$(document).ready(function () {
  ...
  $('.poliklinik').selectator({
    ...
    load: function (search, callback) {
      ...
      $.ajax({
        url: 'http://localhost/Khanza-Lite/admin/pendaftaran/ajax?show=poliklinik&s=' + encodeURIComponent(search) + '&t=05478e6e8c72',
        type: 'GET',
        dataType: 'json',
  ...
  $('.dokter').selectator({
    ...
    load: function (search, callback) {
      if (search.length < this.minSearchLength) return callback();
      $.ajax({
        url: 'http://localhost/Khanza-Lite/admin/pendaftaran/ajax?show=dokter&s=' + encodeURIComponent(search) + '&t=05478e6e8c72',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
  ...
  $('.icd10').selectator({
    ...
    load: function (search, callback) {
      if (search.length < this.minSearchLength) return callback();
      $.ajax({
        url: 'http://localhost/Khanza-Lite/admin/pendaftaran/ajax?show=diagnosa&s=' + encodeURIComponent(search) + '&t=05478e6e8c72',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
  ...
  $('.propinsi').selectator({
    ...
    load: function (search, callback) {
      if (search.length < this.minSearchLength) return callback();
      $.ajax({
        url: 'http://localhost/Khanza-Lite/admin/pendaftaran/ajax?show=propinsi&t=05478e6e8c72',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
          callback(data['response']['list'].slice(0, 100));
  ...
  $('.kabupaten').selectator({
    ...
    load: function (search, callback) {
      if (search.length < this.minSearchLength) return callback();
      $.ajax({
        url: 'http://localhost/Khanza-Lite/admin/pendaftaran/ajax?show=kabupaten&t=05478e6e8c72',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
          callback(data['response']['list'].slice(0, 100));
