...
class AntrianService
{
    ...
    private function _request($method, $feature, $data = [], $headers = [])
    {
        ...
        // var_dump($response['metadata']['message']);
        // exit();
        // if (!empty($response['response']) and $response['metadata']['message'] == 'OK' and is_string($response['response'])) {
        //     return json_decode($this->_decompress($response['response']), true);
        // }
