            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                            <a href="<?php echo URL; ?>/index.php?module=Website&page=add_post" class="btn btn-primary" style="float:right;">Tambah</a>
                            <h2>
                                Post List
                            </h2>
                        </div>
                        <div class="body">
                            <div class="table-responsive">
                                <table class="table table-hover display">
                                    ...
                                    <tbody>
                                      <?php
                                      $sql = "SELECT a.post_id, a.post_title, a.post_date, b.cat_name, a.post_author, a.post_status FROM website_posts a, website_categories b WHERE a.post_cat_id = b.cat_id AND a.post_type = 'post' ORDER BY a.post_id ASC";
                                      $query = query($sql);
                                      $no = '1';
                                      while ($row = fetch_array($query)) {
                                        echo '<tr>';
                                        echo '<th scope="row">'.$no.'</th>';
                                        echo '<td><a href="'.URL.'/index.php?module=Website&page=posts&action=edit&post_id='.$row['post_id'].'">'.$row['post_title'].'</a></td>';
            ...
            if(isset($_GET['action']) == "edit" ) {
                ...
                if($_SERVER['REQUEST_METHOD'] == "POST") {
                  ...
                  if (!empty($errors)) {
                      ...
                      $sql = "UPDATE website_posts SET post_title = '" . $_POST['post_title'] . "', post_content = '" . $_POST['post_content'] . "', post_cat_id = '" . $_POST['post_cat_id'] . "', post_image = '" . $_POST['post_image'] . "', post_status = '" . $_POST['post_status'] . "' WHERE post_id = '" . $_GET['post_id'] . "'";
                      $update = query($sql);
                      if($update) {
                          set_message('Post updated!');
                          redirect(URL.'/index.php?module=Website&page=posts&action=edit&post_id='.$_GET['post_id']);
                      }
                  }
                }
                            ...
                            <form action="" method="POST">
                                ...
                                <div class="form-group">
                                    <div class="form-line">
                                        ...
                                        $query = query($sql);
                                        while ($row = fetch_array($query)) {
                                            $selected = ($result['post_cat_id'] == $row['cat_id'])? "selected" : "";
                                ...
                                <div class="form-group">
                                    <div class="form-line">
                                        ...
                                        <br><br>
                                        <div id="cont-img"><img id="image_preview" src="<?php echo $result['post_image']; ?>" width="200" /></div>
                                    </div>
                                </div>
                                ...
                                <button type="submit" class="btn btn-primary m-t-15 waves-effect">SUBMIT</button> <a href="<?php echo URL; ?>/index.php?module=Website&page=posts&action=delete&post_id=<?php echo $_GET['post_id']; ?>" class="trash" onclick="return confirm(\'Anda yakin ingin menghapus?\');"><button type="button" class="btn btn-danger m-t-15 waves-effect">HAPUS</button>
                            </form>
                        </div>
                    </div>
                </div>
