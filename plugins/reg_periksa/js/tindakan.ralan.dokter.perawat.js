jQuery().ready(function () {
    var var_tbl_rawat_jl_drpr = $('#tindakan_dokter_perawat_reg_periksa #tbl_rawat_jl_drpr').DataTable({
        'processing': true,
        'serverSide': true,
        'serverMethod': 'post',
        'dom': 'Bfrtip',
        ...
        "bInfo" : false,
        "ajax": {
            "url": "{?=url(['reg_periksa','datatindakanralandokterperawat'])?}",
            "dataType": "json",
            "type": "POST",
            "data": function (data) {
    ...
    $.contextMenu({
        selector: '#tbl_rawat_jl_drpr tr', 
        trigger: 'right',
        callback: function(key, options) {
          var rowData = var_tbl_rawat_jl_drpr.rows({ selected: true }).data()[0];
          if (rowData != null) {
var no_rawat = rowData['no_rawat'];
            switch (key) {
                case 'detail' :
                OpenModal(mlite.url + '/rawat_jl_drpr/detail/' + no_rawat + '?t=' + mlite.token);
                break;
                default :
                break
            } 
    ...
    $("form[name='form_rawat_jl_drpr']").validate({
        rules: {
no_rawat: 'required',
...
var stts_bayar= $('#stts_bayar').val();
 ...
 formData.append('typeact', typeact); // tambahan
    ...
    // ==============================================================
    $('#filter_search_rawat_jl_drpr').click(function () {
        var_tbl_rawat_jl_drpr.draw();
    });
    ...
    $("#edit_data_rawat_jl_drpr").click(function () {
        var rowData = var_tbl_rawat_jl_drpr.rows({ selected: true }).data()[0];
        if (rowData != null) {
...
$('#modal_rawat_jl_drpr #stts_bayar').val(stts_bayar).change();
    ...
    // ==============================================================
    jQuery("#hapus_data_rawat_jl_drpr").click(function () {
        var rowData = var_tbl_rawat_jl_drpr.rows({ selected: true }).data()[0];
...
var no_rawat = rowData['no_rawat'];
    ...
    // ==============================================================
    jQuery("#tambah_data_rawat_jl_drpr").click(function () {
...
$('#menejemen').val('');
$('#biaya_rawat').val('');
$('#modal_rawat_jl_drpr #stts_bayar').val('Belum').change();

$('#modal_rawat_jl_drpr #kd_jenis_prw').on('change', function() {
    $.ajax({
        ...
        success: function (data) {
            console.log(data);
            data = JSON.parse(data);
            $('#modal_rawat_jl_drpr #material').val(data.material);
            $('#modal_rawat_jl_drpr #bhp').val(data.bhp);
            $('#modal_rawat_jl_drpr #tarif_tindakandr').val(data.tarif_tindakandr);
            $('#modal_rawat_jl_drpr #tarif_tindakanpr').val(data.tarif_tindakanpr);
            $('#modal_rawat_jl_drpr #kso').val(data.kso);
            $('#modal_rawat_jl_drpr #menejemen').val(data.menejemen);
            $('#modal_rawat_jl_drpr #biaya_rawat').val(data.total_byrdrpr);
        }
    })
}); 
        ...
        $('#modal-title').text("Tambah Data Tindakan Ralan Dokter Perawat");
        $("#modal_rawat_jl_drpr").modal('show');
    });
    ...
    // ===========================================
    $("#lihat_data_rawat_jl_drpr").click(function () {

        var search_field_rawat_jl_drpr = $('#search_field_rawat_jl_drpr').val();
        var search_text_rawat_jl_drpr = $('#search_text_rawat_jl_drpr').val();
        ...
        $.ajax({
            ...
            data: {
                typeact: 'lihat', 
                search_field_rawat_jl_drpr: search_field_rawat_jl_drpr, 
                search_text_rawat_jl_drpr: search_text_rawat_jl_drpr
            },
            dataType: 'json',
            success: function (res) {
                var eTable = "<div class='table-responsive'><table id='tbl_lihat_rawat_jl_drpr' class='table display dataTable' style='width:100%'><thead><th>No Rawat</th><th>Kd Jenis Prw</th><th>Kd Dokter</th><th>Nip</th><th>Tgl Perawatan</th><th>Jam Rawat</th><th>Material</th><th>Bhp</th><th>Tarif Tindakandr</th><th>Tarif Tindakanpr</th><th>Kso</th><th>Menejemen</th><th>Biaya Rawat</th><th>Stts Bayar</th></thead>";
...
eTable += '<td>' + res[i]['stts_bayar'] + '</td>';
                ...
                $('#forTable_rawat_jl_drpr').html(eTable);
            }
        });

        $('#modal-title').text("Lihat Data");
        $("#modal_lihat_rawat_jl_drpr").modal('show');
    });
    ...
    $("#export_pdf").click(function () {
        ...
        doc.setFontSize(14);
        doc.text("Tabel Data Rawat Jl Drpr", 20, 95, null, null, null);
        const totalPagesExp = "{total_pages_count_string}";        
        doc.autoTable({
            html: '#tbl_lihat_rawat_jl_drpr',
            startY: 105,
            margin: {
                left: 20, 
                right: 20
        ...
        // doc.save('table_data_rawat_jl_drpr.pdf')
    ...
    $("#export_xlsx").click(function () {
        let tbl1 = document.getElementById("tbl_lihat_rawat_jl_drpr");
