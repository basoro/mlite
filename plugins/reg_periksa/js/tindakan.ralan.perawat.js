jQuery().ready(function () {
    var var_tbl_rawat_jl_pr = $('#tindakan_perawat_reg_periksa #tbl_rawat_jl_pr').DataTable({
        'processing': true,
        'serverSide': true,
        'serverMethod': 'post',
        'dom': 'Bfrtip',
        ...
        "bInfo" : false,
        "ajax": {
            "url": "{?=url(['reg_periksa','datatindakanralanperawat'])?}",
            "dataType": "json",
            "type": "POST",
            "data": function (data) {
...
var stts_bayar= $('#stts_bayar').val();
 ...
 formData.append('typeact', typeact); // tambahan
    ...
    $("form[name='form_rawat_jl_pr_edit']").validate({
        rules: {
no_rawat: 'required',
kd_jenis_prw: 'required',
nip: 'required',
...
$('#modal_rawat_jl_pr_edit #kd_jenis_prw').val(kd_jenis_prw);
$('#modal_rawat_jl_pr_edit #nip').val(nip).change();
$('#modal_rawat_jl_pr_edit #tgl_perawatan').val(tgl_perawatan);
$('#modal_rawat_jl_pr_edit #jam_rawat').val(jam_rawat);
$('#modal_rawat_jl_pr_edit #material').val(material);
$('#modal_rawat_jl_pr_edit #bhp').val(bhp);
$('#modal_rawat_jl_pr_edit #tarif_tindakanpr').val(tarif_tindakanpr);
$('#modal_rawat_jl_pr_edit #kso').val(kso);
$('#modal_rawat_jl_pr_edit #menejemen').val(menejemen);
$('#modal_rawat_jl_pr_edit #biaya_rawat').val(biaya_rawat);
$('#modal_rawat_jl_pr_edit #stts_bayar').val(stts_bayar).change();

            $("#no_rawat_rawat_jl_pr_edit").prop('readonly', true); // GA BISA DIEDIT KALI DISABLE
            $("#modal_rawat_jl_pr_edit #kd_jenis_prw").prop('readonly', true); // GA BISA DIEDIT KALI DISABLE
            $("#modal_rawat_jl_pr_edit #tgl_perawatan").prop('readonly', true); // GA BISA DIEDIT KALI DISABLE
            $("#modal_rawat_jl_pr_edit #jam_rawat").prop('readonly', true); // GA BISA DIEDIT KALI DISABLE
            $("#modal_rawat_jl_pr_edit #material").prop('readonly', true); // GA BISA DIEDIT KALI DISABLE
            $("#modal_rawat_jl_pr_edit #bhp").prop('readonly', true); // GA BISA DIEDIT KALI DISABLE
            $("#modal_rawat_jl_pr_edit #tarif_tindakanpr").prop('readonly', true); // GA BISA DIEDIT KALI DISABLE
            $("#modal_rawat_jl_pr_edit #kso").prop('readonly', true); // GA BISA DIEDIT KALI DISABLE
            $("#modal_rawat_jl_pr_edit #menejemen").prop('readonly', true); // GA BISA DIEDIT KALI DISABLE
            $("#modal_rawat_jl_pr_edit #biaya_rawat").prop('readonly', true); // GA BISA DIEDIT KALI DISABLE
            $('#modal-title').text("Edit Data Tindakan Ralan Perawat");
            $("#modal_rawat_jl_pr_edit").modal('show');
        }
        else {
...
$('#menejemen').val('');
$('#biaya_rawat').val('');
$('#modal_rawat_jl_pr #stts_bayar').val('Belum').change();

$('#modal_rawat_jl_pr #kd_jenis_prw').on('change', function() {
    $.ajax({
        ...
        success: function (data) {
            console.log(data);
            data = JSON.parse(data);
            $('#modal_rawat_jl_pr #material').val(data.material);
            $('#modal_rawat_jl_pr #bhp').val(data.bhp);
            $('#modal_rawat_jl_pr #tarif_tindakanpr').val(data.tarif_tindakanpr);
            $('#modal_rawat_jl_pr #kso').val(data.kso);
            $('#modal_rawat_jl_pr #menejemen').val(data.menejemen);
            $('#modal_rawat_jl_pr #biaya_rawat').val(data.total_byrdrpr);
        }
    })
}); 
        ...
        $('#modal-title').text("Tambah Data Tindakan Ralan Perawat");
...
eTable += '<td>' + res[i]['stts_bayar'] + '</td>';
    ...
    var tbl_reg_periksa_jns_perawatan_pr = $('#tbl_reg_periksa_jns_perawatan_pr').DataTable({
        columnDefs: [
    ...
    $('#tbl_reg_periksa_jns_perawatan_pr').on('click', 'input[type="checkbox"]', function() {
        var row =  tbl_reg_periksa_jns_perawatan_pr.row($(this).closest('tr'));
        tbl_reg_periksa_jns_perawatan_pr.cell({ row: row.index(), column: 0 } ).data( this.checked ? 1 : 0 )
        row.invalidate().draw()
    })  
