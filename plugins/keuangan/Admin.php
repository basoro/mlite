...
class Admin extends AdminModule
{
    ...
    public function getCashFlow()
    {
      ...
      $rows_aruskas = $this->core->mysql('mlite_arus_kas')->where('id', '<>', '0')->toArray();
      $total_kredit = 0;
      $total_debet = 0;
      ...
      foreach ($rows_aruskas as $row) {
        ...
        ->join('mlite_rekening', 'mlite_rekening.kd_rek=mlite_detailjurnal.kd_rek')
        ->where('arus_kas', $row['id'])
        ->where('kredit', '<>', '0')
        ->group('mlite_detailjurnal.kd_rek')
        ->toArray();
        $row['jurnal_masuk'] = [];
        foreach ($rows_kredit as $row_kredit) {
          $kredits = $this->core->mysql('mlite_detailjurnal')->where('kd_rek', $row_kredit['kd_rek'])->where('arus_kas', $row['id'])->toArray();
        ...
        foreach ($rows_debet as $row_debet) {
          $debets = $this->core->mysql('mlite_detailjurnal')->where('kd_rek', $row_debet['kd_rek'])->where('arus_kas', $row['id'])->toArray();
