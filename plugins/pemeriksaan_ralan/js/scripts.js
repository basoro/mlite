jQuery().ready(function () {
    var var_tbl_pemeriksaan_ralan = $('#tbl_pemeriksaan_ralan').DataTable({
        ...
        "bInfo" : false,
        "ajax": {
            "url": "{?=url(['pemeriksaan_ralan','data'])?}",
            "dataType": "json",
            "type": "POST",
            "data": function (data) {
        ...
        "fnDrawCallback": function () {
            $('#more_data_pemeriksaan_ralan').on('click', function(e) {
                e.preventDefault();
                var clientX = e.originalEvent.clientX;
                var clientY = e.originalEvent.clientY;
                $('#tbl_pemeriksaan_ralan tr').contextMenu({x: clientX, y: clientY});
            });          
        },        
        "columns": [
            { 'data': 'no_rawat' },
    ...
    $("#view_chart").click(function () {
        window.open(mlite.url + '/pemeriksaan_ralan/chart?t=' + mlite.token, '_blank',"toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no,modal=yes");  
    })   
