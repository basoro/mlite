        ...
        if (DBDRIVER === 'sqlite') {
            $core->db()->pdo()->exec("
                CREATE TABLE IF NOT EXISTS mlite_news_tags_relationship (
                    ...
                    FOREIGN KEY (news_id) REFERENCES mlite_news (id) ON DELETE CASCADE ON UPDATE NO ACTION,
                    FOREIGN KEY (tag_id) REFERENCES mlite_news_tags (id) ON DELETE CASCADE ON UPDATE NO ACTION
                )
            ");
        } else {
