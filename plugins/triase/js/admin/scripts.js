...
eTable += '<td>' + res[i]['nik'] + '</td>';
    ...
    jQuery("#lihat_detail_mlite_triase2").click(function (event) {
        ...
        if (rowData) {
            ...
            {if: $this->core->ActiveModule('jasper')}
                var loadURL =  baseURL + '/jasper/triase/' + no_rawat.replace(/\//g,'') + '?t=' + mlite.token;
                $("#modal_detail_mlite_triase").modal('show').html('<div style="text-align:center;margin:20px auto;width:90%;height:95%;"><iframe src="' + loadURL + '" frameborder="no" width="100%" height="100%"></iframe></div>');
            {else}
