jQuery().ready(function () {
    var var_tbl_jns_perawatan = $('#tbl_jns_perawatan').DataTable({
        'processing': true,
        'serverSide': true,
        'serverMethod': 'post',
        'dom': 'Bfrtip',
        ...
        "bInfo" : false,
        "ajax": {
            "url": "{?=url(['jns_perawatan','data'])?}",
            "dataType": "json",
            "type": "POST",
            "data": function (data) {

                // Read values
                var search_field_jns_perawatan = $('#search_field_jns_perawatan').val();
                var search_text_jns_perawatan = $('#search_text_jns_perawatan').val();
                
                data.search_field_jns_perawatan = search_field_jns_perawatan;
                data.search_text_jns_perawatan = search_text_jns_perawatan;
                
            }
        },
        "fnDrawCallback": function () {
            $('#more_data_jns_perawatan').on('click', function(e) {
                e.preventDefault();
                var clientX = e.originalEvent.clientX;
                var clientY = e.originalEvent.clientY;
                $('#tbl_jns_perawatan tr').contextMenu({x: clientX, y: clientY});
            });          
        },        
        "columns": [
            { 'data': 'kd_jenis_prw' },
    ...
    $.contextMenu({
        selector: '#tbl_jns_perawatan tr', 
        trigger: 'right',
        callback: function(key, options) {
          var rowData = var_tbl_jns_perawatan.rows({ selected: true }).data()[0];
          if (rowData != null) {
            var kd_jenis_prw = rowData['kd_jenis_prw'];
            switch (key) {
                case 'detail' :
                    OpenModal(mlite.url + '/jns_perawatan/detail/' + kd_jenis_prw + '?t=' + mlite.token);
                break;
                default :
                break
            } 
    ...
    $("form[name='form_jns_perawatan']").validate({
        rules: {
                ...
                success: function (data) {
                    ...
                    if (typeact == "add") {
                        if(data.status === 'success') {
                            ...
                            $("#modal_jns_perawatan").modal('hide');
                        } else {
    ...
    // ==============================================================
    $('#filter_search_jns_perawatan').click(function () {
        var_tbl_jns_perawatan.draw();
    });
    ...
    $("#edit_data_jns_perawatan").click(function () {
        var rowData = var_tbl_jns_perawatan.rows({ selected: true }).data()[0];
        if (rowData != null) {
            ...
            $('#modal-title').text("Edit Data Jns Perawatan");
            $("#modal_jns_perawatan").modal('show');
        }
        else {
    ...
    // ==============================================================
    jQuery("#hapus_data_jns_perawatan").click(function () {
        var rowData = var_tbl_jns_perawatan.rows({ selected: true }).data()[0];
    ...
    // ==============================================================
    jQuery("#tambah_data_jns_perawatan").click(function () {

        $('#kd_jenis_prw').val('{?=$this->core->setKodeJnsPerawatan()?}');
        $('#nm_perawatan').val('');
        $('#kd_kategori').val('');
        $('#material').val('0');
        $('#bhp').val('0');
        ...
        $('#modal-title').text("Tambah Data Jns Perawatan");
        $("#modal_jns_perawatan").modal('show');
    });
    ...
    // ===========================================
    $("#lihat_data_jns_perawatan").click(function () {
        ...
        $.ajax({
            ...
            data: {
                typeact: 'lihat', 
                search_field_jns_perawatan: search_field_jns_perawatan, 
                search_text_jns_perawatan: search_text_jns_perawatan
            },
            dataType: 'json',
            success: function (res) {
                var eTable = "<div class='table-responsive'><table id='tbl_lihat_jns_perawatan' class='table display dataTable' style='width:100%'><thead><th>Kd Jenis Prw</th><th>Nm Perawatan</th><th>Kd Kategori</th><th>Material</th><th>Bhp</th><th>Tarif Tindakandr</th><th>Tarif Tindakanpr</th><th>Kso</th><th>Menejemen</th><th>Total Byrdr</th><th>Total Byrpr</th><th>Total Byrdrpr</th><th>Kd Pj</th><th>Kd Poli</th><th>Status</th></thead>";
                ...
                $('#forTable_jns_perawatan').html(eTable);
            }
        });

        $('#modal-title').text("Lihat Data");
        $("#modal_lihat_jns_perawatan").modal('show');
    });
    ...
    $("#export_pdf").click(function () {
        ...
        doc.setFontSize(14);
        doc.text("Tabel Data Jns Perawatan", 20, 95, null, null, null);
        const totalPagesExp = "{total_pages_count_string}";        
        doc.autoTable({
            html: '#tbl_lihat_jns_perawatan',
            startY: 105,
            margin: {
                left: 20, 
                right: 20
        ...
        // doc.save('table_data_jns_perawatan.pdf');
    ...
    $("#export_xlsx").click(function () {
        let tbl1 = document.getElementById("tbl_lihat_jns_perawatan");
        let worksheet_tmp1 = XLSX.utils.table_to_sheet(tbl1);
        let a = XLSX.utils.sheet_to_json(worksheet_tmp1, { header: 1 });
        let worksheet1 = XLSX.utils.json_to_sheet(a, { skipHeader: true });
        const new_workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(new_workbook, worksheet1, "Data jns_perawatan");
        XLSX.writeFile(new_workbook, 'tmp_file.xls');
    })

    $("#view_chart").click(function () {
        window.open(mlite.url + '/jns_perawatan/chart?t=' + mlite.token, '_blank',"toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no,modal=yes");  
    })   
