jQuery().ready(function () {
    var var_tbl_jadwal_pegawai = $('#tbl_jadwal_pegawai').DataTable({
        'processing': true,
        'serverSide': true,
        'serverMethod': 'post',
        'dom': 'Bfrtip',
        ...
        "bInfo" : false,
        "ajax": {
            "url": "{?=url(['jadwal_pegawai','data'])?}",
            "dataType": "json",
            "type": "POST",
            "data": function (data) {

                // Read values
                var search_field_jadwal_pegawai = $('#search_field_jadwal_pegawai').val();
                var search_text_jadwal_pegawai = $('#search_text_jadwal_pegawai').val();
                
                data.search_field_jadwal_pegawai = search_field_jadwal_pegawai;
                data.search_text_jadwal_pegawai = search_text_jadwal_pegawai;
                
            }
        },
        "fnDrawCallback": function () {
            $('#more_data_jadwal_pegawai').on('click', function(e) {
                e.preventDefault();
                var clientX = e.originalEvent.clientX;
                var clientY = e.originalEvent.clientY;
                $('#tbl_jadwal_pegawai tr').contextMenu({x: clientX, y: clientY});
            });          
        }, 
        "columns": [
{ 'data': 'id' },
...
{ 'targets': 33}
    ...
    $.contextMenu({
        selector: '#tbl_jadwal_pegawai tr', 
        trigger: 'right',
        callback: function(key, options) {
          var rowData = var_tbl_jadwal_pegawai.rows({ selected: true }).data()[0];
          if (rowData != null) {
var id = rowData['id'];
            switch (key) {
                case 'detail' :
                    OpenModal(mlite.url + '/jadwal_pegawai/detail/' + id + '?t=' + mlite.token);
                break;
                default :
                break
            } 
    ...
    $("form[name='form_jadwal_pegawai']").validate({
        rules: {
id: 'required',
tahun: 'required',
bulan: 'required',
...
formData.append('typeact', typeact); // tambahan
                ...
                success: function (data) {
                    ...
                    if (typeact == "add") {
                        if(data.status === 'success') {
                            ...
                            $("#modal_jadwal_pegawai").modal('hide');
                        } else {
    ...
    // ==============================================================
    $('#filter_search_jadwal_pegawai').click(function () {
        var_tbl_jadwal_pegawai.draw();
    });
    ...
    $("#edit_data_jadwal_pegawai").click(function () {
        var rowData = var_tbl_jadwal_pegawai.rows({ selected: true }).data()[0];
        if (rowData != null) {
...
$('#h31').val(h31);
            ...
            $("#modal_jadwal_pegawai").modal('show');
        }
        else {
    ...
    // ==============================================================
    jQuery("#hapus_data_jadwal_pegawai").click(function () {
        var rowData = var_tbl_jadwal_pegawai.rows({ selected: true }).data()[0];
...
var id = rowData['id'];
    ...
    // ==============================================================
    jQuery("#tambah_data_jadwal_pegawai").click(function () {
...
$('#h31').val('');
    ...
    // ===========================================
    $("#lihat_data_jadwal_pegawai").click(function () {
...
eTable += '<td>' + res[i]['h31'] + '</td>';
                ...
                $('#forTable_jadwal_pegawai').html(eTable);
            }
        });

        $('#modal-title').text("Lihat Data");
        $("#modal_lihat_jadwal_pegawai").modal('show');
    });
    ...
    $("#export_pdf").click(function () {
        ...
        doc.setFontSize(14);
        doc.text("Tabel Data Jadwal Pegawai", 20, 95, null, null, null);
        const totalPagesExp = "{total_pages_count_string}";        
        doc.autoTable({
            html: '#tbl_lihat_jadwal_pegawai',
            startY: 105,
            margin: {
                left: 20, 
                right: 20
        ...
        // doc.save('table_data_jadwal_pegawai.pdf');
    ...
    $("#export_xlsx").click(function () {
        let tbl1 = document.getElementById("tbl_lihat_jadwal_pegawai");
        let worksheet_tmp1 = XLSX.utils.table_to_sheet(tbl1);
        let a = XLSX.utils.sheet_to_json(worksheet_tmp1, { header: 1 });
        let worksheet1 = XLSX.utils.json_to_sheet(a, { skipHeader: true });
        const new_workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(new_workbook, worksheet1, "Data jadwal_pegawai");
        XLSX.writeFile(new_workbook, 'tmp_file.xls');
    })

    $("#view_chart").click(function () {
        window.open(mlite.url + '/jadwal_pegawai/chart?t=' + mlite.token, '_blank',"toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no,modal=yes");  
    })   
