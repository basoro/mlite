-- Seeder dummy Clinical Pathway Dengue Ranap
-- Mencakup:
...
SET @adult_no_rawat = '2026/06/14/000401';
SET @child_no_rawat = '2026/06/14/000402';
SET @adult_rm = '940401';
SET @child_rm = '940402';
SET @adult_cp_code = 'CP-DENGUE-RANAP-DWS';
SET @child_cp_code = 'CP-DENGUE-RANAP-ANK';
SET @adult_start = '2026-06-14 08:00:00';
SET @child_start = '2026-06-14 09:00:00';
SET @adult_recipe = '260614040101';
SET @child_recipe = '260614040102';
...
SELECT 'A91', 'Dengue haemorrhagic fever', 'Demam dengue', 'Seeder dummy Clinical Pathway dengue ranap', NULL, 'Menular'
FROM DUAL
WHERE NOT EXISTS (
  SELECT 1
  FROM penyakit
  WHERE kd_penyakit = 'A91'
);
...
SELECT 'LABDG001', 'Hematologi Lengkap Dengue', 0, 0, 0, 85000, 0, 0, 0, 85000, '-', '1', 'Kelas 1', 'PK'
FROM DUAL
WHERE NOT EXISTS (
  SELECT 1 FROM jns_perawatan_lab WHERE kd_jenis_prw = 'LABDG001'
);

INSERT INTO jns_perawatan_lab
(kd_jenis_prw, nm_perawatan, bagian_rs, bhp, tarif_perujuk, tarif_tindakan_dokter, tarif_tindakan_petugas, kso, menejemen, total_byr, kd_pj, status, kelas, kategori)
SELECT 'LABDG002', 'Trombosit dan Hematokrit Serial', 0, 0, 0, 125000, 0, 0, 0, 125000, '-', '1', 'Kelas 1', 'PK'
FROM DUAL
WHERE NOT EXISTS (
  SELECT 1 FROM jns_perawatan_lab WHERE kd_jenis_prw = 'LABDG002'
);
...
SELECT 'RADDG001', 'USG Abdomen Dengue', 0, 0, 0, 150000, 0, 0, 0, 150000, '-', '1', 'Kelas 1'
FROM DUAL
WHERE NOT EXISTS (
  SELECT 1 FROM jns_perawatan_radiologi WHERE kd_jenis_prw = 'RADDG001'
);
...
VALUES
(
  @adult_rm, 'Rina Marlina', '6301000000940401', 'L', 'Barabai', '1990-01-15', 'Siti Aminah',
  'Jl. Murakata No. 20', 'O', 'Wiraswasta', 'MENIKAH', 'ISLAM', '2026-06-14', '081111110201',
  '36 Th', 'SMA', 'ISTRI', 'Siti Badriah', 'UMU', NULL, '1', 1, 1,
  'Ibu Rumah Tangga', 'Jl. Murakata No. 20', '-', '-', '-', '-', 1, 1, 1,
  'rina.dengue@example.com', '-', 1, '-'
),
(
  @child_rm, 'Dewi Anggraini', '6301000000940402', 'P', 'Barabai', '2016-05-12', 'Nurhayati',
  'Jl. Perintis No. 30', 'A', 'Pelajar', 'BELUM MENIKAH', 'ISLAM', '2026-06-14', '082222220202',
  '10 Th', 'SD', 'IBU', 'Nurhayati', 'UMU', NULL, '1', 1, 1,
  'Ibu Rumah Tangga', 'Jl. Perintis No. 30', '-', '-', '-', '-', 1, 1, 1,
  'dewi.dengue@example.com', '-', 1, '-'
);
...
VALUES
(
  'T00401', @adult_no_rawat, '2026-06-14', '08:00:00', @kd_dokter, @adult_rm, 'UMU',
  'Siti Badriah', 'Jl. Murakata No. 20', 'ISTRI', 0, 'Dirawat', 'Baru', 'Ranap',
  'UMU', 36, 'Th', 'Belum Bayar', 'Baru'
),
(
  'T00402', @child_no_rawat, '2026-06-14', '09:00:00', @kd_dokter, @child_rm, 'UMU',
...
VALUES
(@adult_no_rawat, 'A91', 'Ranap', 1, 'Baru'),
(@child_no_rawat, 'A91', 'Ranap', 1, 'Baru');
...
VALUES
(
  @adult_cp_code, 'Clinical Pathway Dengue Dewasa Ranap', 'Ranap', 4, 3500000, 88.40,
...
(
  @child_cp_code, 'Clinical Pathway Dengue Anak Ranap', 'Ranap', 4, 3000000, 86.75,
...
VALUES
(@adult_cp_id, 'A91', 1, 'Utama'),
(@child_cp_id, 'A91', 1, 'Utama');
...
((SELECT id FROM mlite_clinical_pathway_day WHERE clinical_pathway_id = @adult_cp_id AND hari_ke = 1), 'Laboratorium', 'periksa_lab', 'LABDG001', 'Hematologi Lengkap Dengue', 92, 94.00, 'Wajib', 'Ya', 20),
((SELECT id FROM mlite_clinical_pathway_day WHERE clinical_pathway_id = @adult_cp_id AND hari_ke = 1), 'Laboratorium', 'periksa_lab', 'LABDG002', 'Trombosit dan Hematokrit Serial', 78, 76.00, 'Direkomendasikan', 'Tidak', 30),
...
((SELECT id FROM mlite_clinical_pathway_day WHERE clinical_pathway_id = @adult_cp_id AND hari_ke = 1), 'Obat', 'resep_obat', @kode_obat_sefalosporin, 'Oralit Sachet', 88, 89.00, 'Wajib', 'Ya', 50),
...
((SELECT id FROM mlite_clinical_pathway_day WHERE clinical_pathway_id = @adult_cp_id AND hari_ke = 2), 'Radiologi', 'periksa_radiologi', 'RADDG001', 'USG Abdomen Dengue', 54, 52.00, 'Direkomendasikan', 'Tidak', 10),
...
((SELECT id FROM mlite_clinical_pathway_day WHERE clinical_pathway_id = @adult_cp_id AND hari_ke = 3), 'Laboratorium', 'periksa_lab', 'LABDG001', 'Evaluasi darah rutin ulang bila perlu', 44, 38.00, 'Opsional', 'Tidak', 10),
((SELECT id FROM mlite_clinical_pathway_day WHERE clinical_pathway_id = @adult_cp_id AND hari_ke = 3), 'Obat', 'manual', '', 'Monitoring fase kritis dan warning sign', 64, 61.00, 'Direkomendasikan', 'Tidak', 20),
...
((SELECT id FROM mlite_clinical_pathway_day WHERE clinical_pathway_id = @child_cp_id AND hari_ke = 1), 'Laboratorium', 'periksa_lab', 'LABDG001', 'Hematologi Lengkap Dengue', 88, 90.00, 'Wajib', 'Ya', 20),
...
((SELECT id FROM mlite_clinical_pathway_day WHERE clinical_pathway_id = @child_cp_id AND hari_ke = 1), 'Obat', 'resep_obat', @kode_obat_sefalosporin, 'Oralit Sachet sesuai berat badan', 86, 88.00, 'Wajib', 'Ya', 40),
...
((SELECT id FROM mlite_clinical_pathway_day WHERE clinical_pathway_id = @child_cp_id AND hari_ke = 2), 'Laboratorium', 'periksa_lab', 'LABDG002', 'Trombosit dan Hematokrit Serial', 72, 74.00, 'Direkomendasikan', 'Tidak', 10),
...
((SELECT id FROM mlite_clinical_pathway_day WHERE clinical_pathway_id = @child_cp_id AND hari_ke = 3), 'Radiologi', 'periksa_radiologi', 'RADDG001', 'USG Abdomen Dengue', 34, 30.00, 'Opsional', 'Tidak', 10),
...
VALUES
(@adult_no_rawat, 'RI001', @nip_perawat, '2026-06-14', '08:35:00', 0, 0, 25000, 0, 0, 25000),
(@child_no_rawat, 'RI001', @nip_perawat, '2026-06-14', '09:20:00', 0, 0, 25000, 0, 0, 25000);
...
VALUES
(@adult_no_rawat, @nip_perawat, 'LABDG001', '2026-06-14', '09:10:00', @kd_dokter, 0, 0, 0, 85000, 0, 0, 0, 85000, @kd_dokter, 'Ranap', 'PK'),
(@adult_no_rawat, @nip_perawat, 'LABDG002', '2026-06-14', '09:20:00', @kd_dokter, 0, 0, 0, 125000, 0, 0, 0, 125000, @kd_dokter, 'Ranap', 'PK'),
(@child_no_rawat, @nip_perawat, 'LABDG001', '2026-06-14', '10:00:00', @kd_dokter, 0, 0, 0, 85000, 0, 0, 0, 85000, @kd_dokter, 'Ranap', 'PK'),
(@child_no_rawat, @nip_perawat, 'LABDG002', '2026-06-13', '08:45:00', @kd_dokter, 0, 0, 0, 125000, 0, 0, 0, 125000, @kd_dokter, 'Ranap', 'PK');
...
VALUES
(@adult_no_rawat, @nip_perawat, 'RADDG001', '2026-06-13', '10:15:00', @kd_dokter, 0, 0, 0, 150000, 0, 0, 0, 150000, @kd_dokter, 'Ranap', 'AP', '55', '8', '100', '0', '0', '1', '0.2'),
(@child_no_rawat, @nip_perawat, 'RADDG001', '2026-06-14', '09:40:00', @kd_dokter, 0, 0, 0, 150000, 0, 0, 0, 150000, @kd_dokter, 'Ranap', 'AP', '50', '6', '90', '0', '0', '1', '0.1');
...
VALUES
(@adult_recipe, '2026-06-14', '08:45:00', @adult_no_rawat, @kd_dokter, '2026-06-14', '08:45:00', 'ranap', '2026-06-14', '10:00:00'),
(@child_recipe, '2026-06-14', '09:30:00', @child_no_rawat, @kd_dokter, '2026-06-14', '09:30:00', 'ranap', '2026-06-14', '10:30:00');
...
VALUES
(@adult_no_rawat, @adult_cp_id, 'A91', @adult_start, NULL, 'Aktif', 'Ya'),
(@child_no_rawat, @child_cp_id, 'A91', @child_start, NULL, 'Aktif', 'Ya');
...
(
  ...
  'USG Abdomen Dengue hari ke-3 tidak dilakukan karena kondisi anak stabil tanpa sesak.',
  'Rendah', NOW(), 'Open'
);
...
VALUES
(@adult_cpp_id, @adult_cp_id, 'seed_generate_cp', @adult_no_rawat, 'Seeder dummy CP dengue ranap dewasa dengan data penunjang lengkap.', 'system', NOW()),
(@child_cpp_id, @child_cp_id, 'seed_generate_cp', @child_no_rawat, 'Seeder dummy CP dengue ranap anak dengan data penunjang lengkap.', 'system', NOW());
