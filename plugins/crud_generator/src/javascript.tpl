...
COLUMNDEFS_ISI
    ...
    $.contextMenu({
        selector: '#tbl_NAMA_TABLE tbody tr', 
        ...
        callback: function(key, options) {
            ...
            let table = $('#tbl_NAMA_TABLE').DataTable();
            let data = table.row(this).data();
            
            // Handle menu actions
            switch(key) {
                case "edit":
                    $('#edit_data_NAMA_TABLE').trigger('click');
                    break;
                case "delete":
                    $('#hapus_data_NAMA_TABLE').trigger('click');
                    break;
                case "detail":
                    $('#lihat_detail_NAMA_TABLE').trigger('click');
                    break;
            }
        },
        items: {
    ...
    $('#tbl_NAMA_TABLE tbody').on('touchstart', 'tr', function(e) {
        let row = $(this);
...
DELETE_ISI
    ...
    if(window.location.search.indexOf('no_rawat') !== -1) { 
        $('#search_text_NAMA_TABLE').val(searchParams.get('no_rawat'));
        var_tbl_NAMA_TABLE.draw();
...
DATA_ISI
    ...
    $("#view_chart").click(function () {
        var baseURL = mlite.url + '/' + mlite.admin;
        window.open(baseURL + '/MODULE_NAME/chart?t=' + mlite.token, '_blank',"toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no,modal=yes");  
    })   
