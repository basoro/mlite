...
class Admin extends AdminModule
{
    ...
    public function postSaveAddTable()
    {
      ...
      if (defined('DBDRIVER') && DBDRIVER == 'sqlite') {
        ...
        $c_query .= "id INTEGER PRIMARY KEY AUTOINCREMENT, ";
    ...
    public function postTable()
    {
      ...
      if (defined('DBDRIVER') && DBDRIVER == 'sqlite') {
        ...
        $data = array();
        foreach($result as $row){
          $data[] = array( 'COLUMN_NAME'=>$row['name'] );
        }
      } else {
        $database = DBNAME;
        $get_table = $this->db()->pdo()->prepare("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='$database' AND TABLE_NAME='$nama_table' ORDER BY ordinal_position ASC");
        $get_table->execute();
        $result = $get_table->fetchAll();
    ...
    public function postTulisChart()
    {
      ...
      $data = $_POST['content']; 
      $f = fopen(MODULES."/".$_POST['modulename']."/view/admin/chart.html", 'w+');
      fwrite($f, $data);
      fclose($f);
      exit();
    } 
