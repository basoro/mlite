jQuery().ready(function () {
    var var_tbl_mlite_penilaian_medis_ranap = $('#tbl_mlite_penilaian_medis_ranap').DataTable({
        'processing': true,
        'serverSide': true,
        'serverMethod': 'post',
        'dom': 'Bfrtip',
        ...
        "bInfo" : false,
        "ajax": {
            "url": "{?=url([ADMIN,'penilaian_medis_ranap','data'])?}",
            "dataType": "json",
            "type": "POST",
            "data": function (data) {

                // Read values
                var search_field_mlite_penilaian_medis_ranap = $('#search_field_mlite_penilaian_medis_ranap').val();
                var search_text_mlite_penilaian_medis_ranap = $('#search_text_mlite_penilaian_medis_ranap').val();
                
                data.search_field_mlite_penilaian_medis_ranap = search_field_mlite_penilaian_medis_ranap;
                data.search_text_mlite_penilaian_medis_ranap = search_text_mlite_penilaian_medis_ranap;
...
{ 'targets': 39}
    ...
    $("form[name='form_mlite_penilaian_medis_ranap']").validate({
        rules: {
no_rawat: 'required',
tanggal: 'required',
kd_dokter: 'required',
...
jantung:'jantung tidak boleh kosong!',
paru:'paru tidak boleh kosong!',
...
var edukasi= $('#edukasi').val();
 ...
 formData.append('typeact', typeact); // tambahan
            ...
            $.ajax({
                url: "{?=url([ADMIN,'penilaian_medis_ranap','aksi'])?}",
                method: "POST",
    ...
    // ==============================================================
    $('#search_text_mlite_penilaian_medis_ranap').keyup(function () {
        var_tbl_mlite_penilaian_medis_ranap.draw();
    });
    // ==============================================================
    // CLICK TANDA X DI INPUT SEARCH
    // ==============================================================
    $("#searchclear_mlite_penilaian_medis_ranap").click(function () {
        $("#search_text_mlite_penilaian_medis_ranap").val("");
        var_tbl_mlite_penilaian_medis_ranap.draw();
    });
    ...
    $("#edit_data_mlite_penilaian_medis_ranap").click(function () {
        var rowData = var_tbl_mlite_penilaian_medis_ranap.rows({ selected: true }).data()[0];
        if (rowData != null) {
...
var jantung = rowData['jantung'];
var paru = rowData['paru'];
...
$('#mata').val(mata).change();
$('#gigi').val(gigi).change();
$('#tht').val(tht).change();
$('#thoraks').val(thoraks).change();
$('#jantung').val(jantung).change();
$('#paru').val(paru).change();
...
$('#edukasi').val(edukasi);
            ...
            $('#modal-title').text("Edit Data Penilaian Medis Ranap");
            $("#modal_mlite_penilaian_medis_ranap").modal();
        }
        else {
    ...
    // ==============================================================
    jQuery("#hapus_data_mlite_penilaian_medis_ranap").click(function () {
        var rowData = var_tbl_mlite_penilaian_medis_ranap.rows({ selected: true }).data()[0];
...
var no_rawat = rowData['no_rawat'];
    ...
    if(window.location.search.indexOf('no_rawat') !== -1) { 
        $('#search_text_mlite_penilaian_medis_ranap').val(searchParams.get('no_rawat'));
        var_tbl_mlite_penilaian_medis_ranap.draw();
    ...
    jQuery("#tambah_data_mlite_penilaian_medis_ranap").click(function () {
...
$('#edukasi').val('');
        ...
        $('#modal-title').text("Tambah Data Penilaian Medis Ranap");
        $("#modal_mlite_penilaian_medis_ranap").modal();
    });
    ...
    // ===========================================
    $("#lihat_data_mlite_penilaian_medis_ranap").click(function () {
        ...
        $.ajax({
            ...
            data: {
                typeact: 'lihat', 
                search_field_mlite_penilaian_medis_ranap: search_field_mlite_penilaian_medis_ranap, 
                search_text_mlite_penilaian_medis_ranap: search_text_mlite_penilaian_medis_ranap
            },
            dataType: 'json',
            success: function (res) {
                var eTable = "<div class='table-responsive'><table id='tbl_lihat_mlite_penilaian_medis_ranap' class='table display dataTable' style='width:100%'><thead><th>No Rawat</th><th>Tanggal</th><th>Kd Dokter</th><th>Anamnesis</th><th>Hubungan</th><th>Keluhan Utama</th><th>Rps</th><th>Rpd</th><th>Rpk</th><th>Rpo</th><th>Alergi</th><th>Keadaan</th><th>Gcs</th><th>Kesadaran</th><th>Td</th><th>Nadi</th><th>Rr</th><th>Suhu</th><th>Spo</th><th>Bb</th><th>Tb</th><th>Kepala</th><th>Mata</th><th>Gigi</th><th>Tht</th><th>Thoraks</th><th>Jantung</th><th>Paru</th><th>Abdomen</th><th>Genital</th><th>Ekstremitas</th><th>Kulit</th><th>Ket Fisik</th><th>Ket Lokalis</th><th>Lab</th><th>Rad</th><th>Penunjang</th><th>Diagnosis</th><th>Tata</th><th>Edukasi</th></thead>";
...
eTable += '<td>' + res[i]['jantung'] + '</td>';
eTable += '<td>' + res[i]['paru'] + '</td>';
...
eTable += '<td>' + res[i]['edukasi'] + '</td>';
                ...
                $('#forTable_mlite_penilaian_medis_ranap').html(eTable);
            }
        });

        $('#modal-title').text("Lihat Data");
        $("#modal_lihat_mlite_penilaian_medis_ranap").modal();
    });

    // ==============================================================
    // TOMBOL DETAIL mlite_penilaian_medis_ranap DI CLICK
    // ==============================================================
    jQuery("#lihat_detail_mlite_penilaian_medis_ranap").click(function (event) {
        ...
        if (rowData) {
            ...
            var loadURL =  baseURL + '/penilaian_medis_ranap/detail/' + no_rawat.replace(/\//g,'') + '?t=' + mlite.token;
        
            var modal = $('#modal_detail_mlite_penilaian_medis_ranap');
            var modalContent = $('#modal_detail_mlite_penilaian_medis_ranap .modal-content');
    ...
    jQuery("#lihat_detail_mlite_penilaian_medis_ranap2").click(function (event) {
        ...
        if (rowData) {
            ...
            {if: $this->core->ActiveModule('jasper')}
                var loadURL =  baseURL + '/jasper/awalmedisranap/' + no_rawat.replace(/\//g,'') + '?t=' + mlite.token;
                $("#modal_detail_mlite_penilaian_medis_ranap").modal('show').html('<div style="text-align:center;margin:20px auto;width:90%;height:95%;"><iframe src="' + loadURL + '" frameborder="no" width="100%" height="100%"></iframe></div>');
            {else}
    ...
    $("#export_pdf").click(function () {
        ...
        doc.setFontSize(14);
        doc.text("Tabel Data Mlite Penilaian Medis Ranap", 20, 95, null, null, null);
        const totalPagesExp = "{total_pages_count_string}";        
        doc.autoTable({
            html: '#tbl_lihat_mlite_penilaian_medis_ranap',
            startY: 105,
            margin: {
                left: 20, 
                right: 20
        ...
        // doc.save('table_data_mlite_penilaian_medis_ranap.pdf')
    ...
    $("#export_xlsx").click(function () {
        let tbl1 = document.getElementById("tbl_lihat_mlite_penilaian_medis_ranap");
        let worksheet_tmp1 = XLSX.utils.table_to_sheet(tbl1);
        let a = XLSX.utils.sheet_to_json(worksheet_tmp1, { header: 1 });
        let worksheet1 = XLSX.utils.json_to_sheet(a, { skipHeader: true });
        const new_workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(new_workbook, worksheet1, "Data mlite_penilaian_medis_ranap");
