...
$("#index").on('click', '#bukaform', function(){
  ...
  $("#form").show().load(baseURL + '/master/sukuform?t=' + mlite.token);
...
$("#form").on("click", "#simpan", function(event){
  ...
  var nama_suku_bangsa = $('input:text[name=nama_suku_bangsa]').val();

  var url = baseURL + '/master/sukusave?t=' + mlite.token;

  $.post(url,{
    id: id,
    nama_suku_bangsa: nama_suku_bangsa
  } ,function(data) {
      $("#display").show().load(baseURL + '/master/sukudisplay?t=' + mlite.token);
      $("#form").hide();
      $("#tutupform").val("Buka Form");
      $("#tutupform").attr("id", "bukaform");
      $('#notif').html("<div class=\"alert alert-success alert-dismissible fade in\" role=\"alert\" style=\"border-radius:0px;margin-top:-15px;\">"+
      "Data suku bangsa telah disimpan!"+
...
$("#display").on("click", ".edit", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url    = baseURL + '/master/sukuform?t=' + mlite.token;
...
$("#form").on("click","#hapus", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url = baseURL + '/master/sukuhapus?t=' + mlite.token;
  var id = $('input:text[name=id]').val();
  ...
  bootbox.confirm("Apakah Anda yakin ingin menghapus data ini?", function(result){
    ...
    if (result){
      ...
      $.post(url, {
        ...
        "Data suku bangsa telah dihapus!"+
        "<button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-label=\"Close\">&times;</button>"+
        "</div>").show();
      });
    }
...
$('input:text[name=cari]').on('input',function(e){
  var baseURL = mlite.url + '/' + mlite.admin;
  var url    = baseURL + '/master/sukudisplay?t=' + mlite.token;
...
function bersih(){
  ...
  $('input:text[name=nama_suku_bangsa]').val("");
}
