...
$("#index").on('click', '#bukaform', function(){
  ...
  $("#form").show().load(baseURL + '/master/statuskerjaform?t=' + mlite.token);
...
$("#form").on("click", "#simpan", function(event){
  ...
  var url = baseURL + '/master/statuskerjasave?t=' + mlite.token;
  ...
  $.post(url,{
    ...
    indek: indek
      ...
      $("#display").show().load(baseURL + '/master/statuskerjadisplay?t=' + mlite.token);
      $("#form").hide();
      $("#tutupform").val("Buka Form");
      $("#tutupform").attr("id", "bukaform");
      $('#notif').html("<div class=\"alert alert-success alert-dismissible fade in\" role=\"alert\" style=\"border-radius:0px;margin-top:-15px;\">"+
      "Data statuskerja telah disimpan!"+
...
$("#display").on("click", ".edit", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url    = baseURL + '/master/statuskerjaform?t=' + mlite.token;
...
$("#form").on("click","#hapus", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url = baseURL + '/master/statuskerjahapus?t=' + mlite.token;
  var stts = $('input:text[name=stts]').val();
  ...
  bootbox.confirm("Apakah Anda yakin ingin menghapus data ini?", function(result){
    ...
    if (result){
      ...
      $.post(url, {
        ...
        "Data statuskerja telah dihapus!"+
        "<button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-label=\"Close\">&times;</button>"+
        "</div>").show();
      });
    }
...
$('input:text[name=cari]').on('input',function(e){
  var baseURL = mlite.url + '/' + mlite.admin;
  var url    = baseURL + '/master/statuskerjadisplay?t=' + mlite.token;
  ...
  if(cari!="") {
      ...
      $("#display").load(baseURL + '/master/statuskerjadisplay?t=' + mlite.token);
  }
