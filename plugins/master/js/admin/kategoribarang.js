...
$("#index").on('click', '#bukaform', function(){
  ...
  $("#form").show().load(baseURL + '/master/kategoribarangform?t=' + mlite.token);
...
$("#form").on("click", "#simpan", function(event){
  ...
  var url = baseURL + '/master/kategoribarangsave?t=' + mlite.token;

  $.post(url,{
    kode: kode,
    nama: nama
  } ,function(data) {
      $("#display").show().load(baseURL + '/master/kategoribarangdisplay?t=' + mlite.token);
      $("#form").hide();
      $("#tutupform").val("Buka Form");
      $("#tutupform").attr("id", "bukaform");
...
$("#display").on("click", ".edit", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url    = baseURL + '/master/kategoribarangform?t=' + mlite.token;
...
$("#form").on("click","#hapus", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url = baseURL + '/master/kategoribaranghapus?t=' + mlite.token;
  var kode = $('input:text[name=kode]').val();
  ...
  bootbox.confirm("Apakah Anda yakin ingin menghapus data ini?", function(result){
    ...
    if (result){
      ...
      $.post(url, {
        ...
        $("#tutupform").val("Buka Form");
        $("#tutupform").attr("id", "bukaform");
        $("#display").load(baseURL + '/master/kategoribarangdisplay?t=' + mlite.token);
        $('#notif').html("<div class=\"alert alert-danger alert-dismissible fade in\" role=\"alert\" style=\"border-radius:0px;margin-top:-15px;\">"+
        "Data kategori barang telah dihapus!"+
        "<button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-label=\"Close\">&times;</button>"+
        "</div>").show();
      });
    }
...
$('input:text[name=cari]').on('input',function(e){
  var baseURL = mlite.url + '/' + mlite.admin;
  var url    = baseURL + '/master/kategoribarangdisplay?t=' + mlite.token;
