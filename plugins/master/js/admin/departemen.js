...
$("#index").on('click', '#bukaform', function(){
  ...
  $("#form").show().load(baseURL + '/master/departemenform?t=' + mlite.token);
...
$("#form").on("click", "#simpan", function(event){
  ...
  var url = baseURL + '/master/departemensave?t=' + mlite.token;

  $.post(url,{
    dep_id: dep_id,
    nama: nama
  } ,function(data) {
      $("#display").show().load(baseURL + '/master/departemendisplay?t=' + mlite.token);
      $("#form").hide();
      $("#tutupform").val("Buka Form");
      $("#tutupform").attr("id", "bukaform");
      $('#notif').html("<div class=\"alert alert-success alert-dismissible fade in\" role=\"alert\" style=\"border-radius:0px;margin-top:-15px;\">"+
      "Data departemen telah disimpan!"+
...
$("#display").on("click", ".edit", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url    = baseURL + '/master/departemenform?t=' + mlite.token;
  var dep_id  = $(this).attr("data-dep_id");

  $.post(url, {dep_id: dep_id} ,function(data) {
    // tampilkan data
    $("#form").html(data).show();
    $("#bukaform").val("Tutup Form");
    $("#bukaform").attr("id", "tutupform");
...
$("#form").on("click","#hapus", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url = baseURL + '/master/departemenhapus?t=' + mlite.token;
  var dep_id = $('input:text[name=dep_id]').val();
  ...
  bootbox.confirm("Apakah Anda yakin ingin menghapus data ini?", function(result){
    ...
    if (result){
      ...
      $.post(url, {
        ...
        $("#tutupform").val("Buka Form");
        $("#tutupform").attr("id", "bukaform");
        $("#display").load(baseURL + '/master/departemendisplay?t=' + mlite.token);
...
$('input:text[name=cari]').on('input',function(e){
  var baseURL = mlite.url + '/' + mlite.admin;
  var url    = baseURL + '/master/departemendisplay?t=' + mlite.token;
...
function bersih(){
  $('input:text[name=dep_id]').val("").removeAttr('disabled');
