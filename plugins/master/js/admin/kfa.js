$(document).ready(function() {
    ...
    $("#index").on('click', '#bukaform', function() {
        $("#form").show().load(baseURL + '/master/kfaform?t=' + mlite.token);
    ...
    // Save
    $("#form").on("submit", "#kfa_form", function(e) {
        e.preventDefault();
        $.post(baseURL + '/master/kfasave?t=' + mlite.token, $(this).serialize(), function() {
            $(".display").load(baseURL + '/master/kfadisplay?t=' + mlite.token);
            $("#form").hide();
            $("#tutupform").val("Buka Form").attr("id", "bukaform");
    ...
    $(".display").on("click", ".edit", function(e) {
        e.preventDefault();
        var kode_kfa = $(this).data("kode_kfa");
        $("#form").load(baseURL + '/master/kfaform?t=' + mlite.token, {kode_kfa: kode_kfa}, function() {
            $(this).show();
    ...
    $(".display").on("click", ".hapus", function(e) {
        ...
        bootbox.confirm("Apakah Anda yakin ingin menghapus data ini?", function(result) {
            if (result) {
                $.post(baseURL + '/master/kfahapus?t=' + mlite.token, {kode_kfa: kode_kfa}, function() {
                    $(".display").load(baseURL + '/master/kfadisplay?t=' + mlite.token);
    ...
    $(".display").on("click", ".halaman", function(e) {
        ...
        $(".display").load(baseURL + '/master/kfadisplay?t=' + mlite.token, {halaman: halaman, cari: cari});
    });
    ...
    $('input:text[name=cari]').on('input', function() {
        ...
        if (cari.length >= 3 || cari.length === 0) {
            $(".display").load(baseURL + '/master/kfadisplay?t=' + mlite.token, {cari: cari});
        }
    });
});
