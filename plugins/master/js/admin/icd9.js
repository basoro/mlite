...
$("#index").on('click', '#bukaform', function(){
  ...
  $("#form").show().load(baseURL + '/master/icd9form?t=' + mlite.token);
...
$("#form").on("click", "#simpan", function(event){
  ...
  var deskripsi_panjang = $('textarea[name=deskripsi_panjang]').val();
  var deskripsi_pendek = $('input:text[name=deskripsi_pendek]').val();

  var url = baseURL + '/master/icd9save?t=' + mlite.token;
  ...
  $.post(url,{
    ...
    deskripsi_pendek: deskripsi_pendek
      ...
      $("#display").show().load(baseURL + '/master/icd9display?t=' + mlite.token);
      $("#form").hide();
      $("#tutupform").val("Buka Form");
      $("#tutupform").attr("id", "bukaform");
...
$("#display").on("click", ".edit", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url    = baseURL + '/master/icd9form?t=' + mlite.token;
...
$("#form").on("click","#hapus", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url = baseURL + '/master/icd9hapus?t=' + mlite.token;
  var kode = $('input:text[name=kode]').val();
  ...
  bootbox.confirm("Apakah Anda yakin ingin menghapus data ini?", function(result){
    ...
    if (result){
      ...
      $.post(url, {
        ...
        $("#tutupform").val("Buka Form");
        $("#tutupform").attr("id", "bukaform");
        $("#display").load(baseURL + '/master/icd9display?t=' + mlite.token);
...
$('input:text[name=cari]').on('input',function(e){
  var baseURL = mlite.url + '/' + mlite.admin;
  var url    = baseURL + '/master/icd9display?t=' + mlite.token;
...
function bersih(){
  ...
  $('textarea[name=deskripsi_panjang]').val("");
  $('input:text[name=deskripsi_pendek]').val("");
}