<?php
    ...
    class Admin extends AdminModule
    {
        ...
        public function getDokterAdd()
        {
            ...
            $this->assign['jk'] = ['L','P'];
            $this->assign['gol_drh'] = ['A', 'B', 'O', 'AB', '-'];
            $this->assign['agama'] = ['ISLAM', 'KRISTEN', 'PROTESTAN', 'HINDU', 'BUDHA', 'KONGHUCU', 'KEPERCAYAAN'];
            $this->assign['stts_nikah'] = ['BELUM MENIKAH','MENIKAH','JANDA','DUDHA','JOMBLO'];
        ...
        public function getPetugasAdd()
        {
            ...
            $this->assign['jk'] = ['L','P'];
            $this->assign['gol_darah'] = ['A', 'B', 'O', 'AB', '-'];
            $this->assign['agama'] = ['ISLAM', 'KRISTEN', 'PROTESTAN', 'HINDU', 'BUDHA', 'KONGHUCU', 'KEPERCAYAAN'];
            $this->assign['stts_nikah'] = ['BELUM MENIKAH','MENIKAH','JANDA','DUDHA','JOMBLO'];
        ...
        /* Master Jns_Perawatan_Ranap Section */
        public function getJnsPerawatanRanap($page = 1)
        {
            $this->_addHeaderFiles();
            $perpage = '10';
            $phrase = '';
            ...
            $totalRecords = $this->db('jns_perawatan_inap')
                ...
                ->orLike('nm_perawatan', '%'.$phrase.'%')
                ->toArray();
            $pagination = new \Systems\Lib\Pagination($page, count($totalRecords), 10, url([ADMIN, 'master', 'jnsperawatanranap', '%d']));
            $this->assign['pagination'] = $pagination->nav('pagination','5');
            $this->assign['totalRecords'] = $totalRecords;

            // list
            $offset = $pagination->offset();
            $rows = $this->db('jns_perawatan_inap')
                ->where('status', $status)
                ->like('kd_jenis_prw', '%'.$phrase.'%')
                ->orLike('nm_perawatan', '%'.$phrase.'%')
                ->offset($offset)
            ...
            if (count($rows)) {
                foreach ($rows as $row) {
                    $row = htmlspecialchars_array($row);
                    $row['editURL'] = url([ADMIN, 'master', 'jnsperawatanranapedit', $row['kd_jenis_prw']]);
                    $row['delURL']  = url([ADMIN, 'master', 'jnsperawatanranapdelete', $row['kd_jenis_prw']]);
                    $row['restoreURL']  = url([ADMIN, 'master', 'jnsperawatanranaprestore', $row['kd_jenis_prw']]);
                    $row['viewURL'] = url([ADMIN, 'master', 'jnsperawatanranapview', $row['kd_jenis_prw']]);
                    $this->assign['list'][] = $row;
                }
            }
            ...
            $this->assign['title'] = 'Kelola Jenis Perawatan';
            $this->assign['getStatus'] = isset($_GET['status']);
            $this->assign['addURL'] = url([ADMIN, 'master', 'jnsperawatanranapadd']);
            $this->assign['printURL'] = url([ADMIN, 'master', 'jnsperawatanranapprint']);

            return $this->draw('jnsperawatanranap.manage.html', ['jnsperawatanranap' => $this->assign]);

        }

        public function getJnsPerawatanRanapAdd()
        {
            $this->_addHeaderFiles();
            if (!empty($redirectData = getRedirectData())) {
                $this->assign['form'] = filter_var_array($redirectData, FILTER_SANITIZE_STRING);
            ...
            $this->assign['kd_kategori'] = $this->db('kategori_perawatan')->toArray();
            $this->assign['kd_pj'] = $this->db('penjab')->toArray();
            $this->assign['kd_bangsal'] = $this->db('bangsal')->toArray();
            $this->assign['kelas'] = ['-','Kelas 1','Kelas 2','Kelas 3','Kelas Utama','Kelas VIP','Kelas VVIP'];

            return $this->draw('jnsperawatanranap.form.html', ['jnsperawatanranap' => $this->assign]);
        }

        public function getJnsPerawatanRanapEdit($id)
        {
            $this->_addHeaderFiles();
            $row = $this->db('jns_perawatan_inap')->where('kd_jenis_prw', $id)->oneArray();
            if (!empty($row)) {
                $this->assign['form'] = $row;
                $this->assign['title'] = 'Edit Jenis Perawatan';
                $this->assign['status'] = ['0','1'];
                ...
                redirect(url([ADMIN, 'master', 'jnsperawatanranap']));
            }
        }

        public function getJnsPerawatanRanapDelete($id)
        {
            if ($this->core->db('jns_perawatan_inap')->where('kd_jenis_prw', $id)->update('status', '0')) {
                $this->notify('success', 'Hapus sukses');
            } else {
                $this->notify('failure', 'Hapus gagal');
            }
        ...
        public function getJnsPerawatanRanapRestore($id)
        {
            if ($this->core->db('jns_perawatan_inap')->where('kd_jenis_prw', $id)->update('status', '1')) {
                $this->notify('success', 'Restore sukses');
            } else {
                $this->notify('failure', 'Restore gagal');
            }
        ...
        public function postJnsPerawatanRanapSave($id = null)
        {
            $errors = 0;

            if (!$id) {
                $location = url([ADMIN, 'master', 'jnsperawatanranapadd']);
            } else {
                $location = url([ADMIN, 'master', 'jnsperawatanranapedit', $id]);
            }
            ...
            if (!$errors) {
                ...
                if (!$id) {    // new
                    ...
                    $query = $this->db('jns_perawatan_inap')->where('kd_jenis_prw', $id)->save($_POST);
                }
        ...
        public function getJnsPerawatanRanapPrint()
        {
          $pasien = $this->db('jns_perawatan_inap')->toArray();
          $logo = url().'/'.$this->settings->get('settings.logo');
          ...
          $pdf->Line(10, 30, 345, 30);
          $pdf->Line(10, 31, 345, 31);
          $pdf->Text(10, 40, 'DATA JENIS PERAWATAN RAWAT INAP');
          $pdf->Ln(34);
          $pdf->SetFont('Arial', '', 10);
          $pdf->SetWidths(array(30,75,20,20,20,20,20,25,20,30,35,20));
          $pdf->Row(array('Kd. Perawatan', 'Nama Perawatan', 'B. Material', 'B. BHP', 'B. Dokter', 'B. Perawat', 'KSO', 'Manajemen', 'Ttl. Dokter', 'Ttl. Perawat', 'Ttl. Dokter/Perawat', 'Status'));
        ...
        /* End Master Jns_Perawatan_Ranap Section */
        ...
        public function getJnsPerawatanLabAdd()
        {
            ...
            $this->assign['status'] = ['0','1'];
            $this->assign['kelas'] = ['-','Rawat Jalan','Kelas 1','Kelas 2','Kelas 3','Kelas Utama','Kelas VIP','Kelas VVIP'];
            $this->assign['kd_pj'] = $this->db('penjab')->toArray();
