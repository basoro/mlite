...
class LoincRad
{
    ...
    {
      $totalRecords = $this->db('mlite_loinc_radiologi')->count();
      $offset         = 10;
      $return['halaman']    = 1;
      $return['jml_halaman']    = ceil($totalRecords / $offset);
      $return['jumlah_data']    = $totalRecords;

      $return['list'] = $this->db('mlite_loinc_radiologi')
        ->desc('Code')
        ->limit(10)
        ->toArray();
    ...
    {
        if (isset($_POST['Code'])){
          $return['form'] = $this->db('mlite_loinc_radiologi')->where('Code', $_POST['Code'])->oneArray();
        } else {
          $return['form'] = [
            'Code' => '',
            'Display' => '',
    ...
    public function anyDisplay()
    {
        ...
        if(isset($_POST['cari'])) {
          $query = $this->db('mlite_loinc_radiologi')
    ...
    {
      if (!$this->db('mlite_loinc_radiologi')->where('Code', $_POST['Code'])->oneArray()) {
        $query = $this->db('mlite_loinc_radiologi')->save($_POST);
      } else {
        $query = $this->db('mlite_loinc_radiologi')->where('Code', $_POST['Code'])->save($_POST);
      }
      return $query;
    }
    ...
    {
      return $this->db('mlite_loinc_radiologi')->where('Code', $_POST['Code'])->delete();
    }
