jQuery().ready(function () {
    var var_tbl_bridging_surat_kontrol_bpjs = $('#tbl_bridging_surat_kontrol_bpjs').DataTable({
        ...
        "bInfo" : false,
        "ajax": {
            "url": "{?=url(['bridging_surat_kontrol_bpjs','data'])?}",
            "dataType": "json",
            "type": "POST",
            "data": function (data) {
        ...
        "fnDrawCallback": function () {
            $('#more_data_bridging_surat_kontrol_bpjs').on('click', function(e) {
                e.preventDefault();
                var clientX = e.originalEvent.clientX;
                var clientY = e.originalEvent.clientY;
                $('#tbl_bridging_surat_kontrol_bpjs tr').contextMenu({x: clientX, y: clientY});
            });          
        }, 
        "columns": [
{ 'data': 'no_sep' },
...
{ 'targets': 7}
    ...
    $.contextMenu({
        selector: '#tbl_bridging_surat_kontrol_bpjs tr', 
        trigger: 'right',
...
var no_sep = rowData['no_sep'];
            switch (key) {
                case 'detail' :
                    OpenModal(mlite.url + '/bridging_surat_kontrol_bpjs/detail/' + no_sep + '?t=' + mlite.token);
                break;
                default :
                break
            } 
...
formData.append('typeact', typeact); // tambahan
            ...
            $.ajax({
                url: "{?=url(['bridging_surat_kontrol_bpjs','aksi'])?}",
                method: "POST",
                ...
                success: function (data) {
                    ...
                    if (typeact == "add") {
                        if(data.status === 'success') {
                            ...
                            $("#modal_bridging_surat_kontrol_bpjs").modal('hide');
                        } else {
    ...
    // ==============================================================
    $('#filter_search_bridging_surat_kontrol_bpjs').click(function () {
        var_tbl_bridging_surat_kontrol_bpjs.draw();
    });
...
$('#nm_poli_bpjs').val(nm_poli_bpjs);
            ...
            $('#modal-title').text("Edit Data Bridging Surat Kontrol Bpjs");
            $("#modal_bridging_surat_kontrol_bpjs").modal('show');
        }
        else {
...
$('#nm_poli_bpjs').val('');
        ...
        $('#modal-title').text("Tambah Data Bridging Surat Kontrol Bpjs");
        $("#modal_bridging_surat_kontrol_bpjs").modal('show');
    });
...
eTable += '<td>' + res[i]['nm_poli_bpjs'] + '</td>';
        ...
        $("#modal_lihat_bridging_surat_kontrol_bpjs").modal('show');
    });
    ...
    $("#view_chart").click(function () {
        window.open(mlite.url + '/bridging_surat_kontrol_bpjs/chart?t=' + mlite.token, '_blank',"toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no,modal=yes");  
    })   
