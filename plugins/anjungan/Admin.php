...
class Admin extends AdminModule
{
    ...
    public function getResetAnjunganLoket(){
      ...
      $date = date('Y-m-d');
      $checkAnjungan = $this->core->mysql('mlite__antrian_loket')->where('postdate',$date)->oneArray();
      if($checkAnjungan){
        $this->core->mysql('mlite__antrian_loket')->where('postdate',$date)->delete();
        echo 'Berhasil Reset Antrian Anjungan Loket<br>';
        $checkAnjungan = $this->core->mysql('mlite__antrian_loket')->where('postdate',$date)->oneArray();
        if(!$checkAnjungan){
          $this->core->mysql('mlite__antrian_loket')->save([
            'type' => 'Loket',
            'noantrian' => '1',
            'postdate' => $date,
