...
class Site extends SiteModule
{
    ...
    public function _getIndexPerbaikan($page = 1)
    {
      ...
      // pagination
      $totalRecords = $this->mysql()->pdo()->prepare("SELECT no_rawat FROM mlite_vedika WHERE status = 'Perbaiki' AND (no_rkm_medis LIKE ? OR no_rawat LIKE ? OR nosep LIKE ?) AND tgl_registrasi BETWEEN '$start_date' AND '$end_date'");
      $totalRecords->execute(['%'.$phrase.'%', '%'.$phrase.'%', '%'.$phrase.'%']);
      $totalRecords = $totalRecords->fetchAll();
      ...
      $offset = $pagination->offset();
      $query = $this->mysql()->pdo()->prepare("SELECT * FROM mlite_vedika WHERE status = 'Perbaiki' AND (no_rkm_medis LIKE ? OR no_rawat LIKE ? OR nosep LIKE ?) AND tgl_registrasi BETWEEN '$start_date' AND '$end_date' ORDER BY nosep LIMIT $perpage OFFSET $offset");
      $query->execute(['%'.$phrase.'%', '%'.$phrase.'%', '%'.$phrase.'%']);
      $rows = $query->fetchAll();
      $this->assign['list'] = [];
      if (count($rows)) {
    ...
    public function getPerbaikanExport()
    {
      $start_date = $_GET['start_date'];
      $end_date = $_GET['end_date'];
      $rows = $this->mysql('mlite_vedika')->where('status', 'Perbaiki')->where('tgl_registrasi','>=',$start_date)->where('tgl_registrasi','<=', $end_date)->toArray();
      if(isset($_GET['jenis']) && $_GET['jenis'] == 1) {
        $rows = $this->mysql('mlite_vedika')->where('status', 'Perbaiki')->where('tgl_registrasi','>=',$start_date)->where('tgl_registrasi','<=', $end_date)->where('jenis', 1)->toArray();
      }
      if(isset($_GET['jenis']) && $_GET['jenis'] == 2) {
        $rows = $this->mysql('mlite_vedika')->where('status', 'Perbaiki')->where('tgl_registrasi','>=',$start_date)->where('tgl_registrasi','<=', $end_date)->where('jenis', 2)->toArray();
      }
      $i = 1;
      foreach ($rows as $row) {
        ...
        $get_feedback_bpjs = $this->mysql('mlite_vedika_feedback')->where('nosep', $row['nosep'])->where('username', 'bpjs')->oneArray();
        $row['konfirmasi_bpjs'] = $get_feedback_bpjs['catatan'];
        $get_feedback_rs = $this->mysql('mlite_vedika_feedback')->where('nosep', $row['nosep'])->where('username','!=','bpjs')->oneArray();
    ...
    {
      $this->_addHeaderFiles();
      $pengajuan_ralan = $this->mysql('mlite_vedika')->select(['count' => 'COUNT(DISTINCT no_rawat)'])->where('status', 'Pengajuan')->where('jenis', 2)->oneArray();
      $pengajuan_ranap = $this->mysql('mlite_vedika')->select(['count' => 'COUNT(DISTINCT no_rawat)'])->where('status', 'Pengajuan')->where('jenis', 1)->oneArray();
      $perbaiki = $this->mysql('mlite_vedika')->select(['count' => 'COUNT(DISTINCT no_rawat)'])->where('status', 'Perbaiki')->oneArray();
    ...
    public function getPDF($id)
    {
      if ($this->_loginCheck()) {
        ...
        $this->tpl->set('hasil_radiologi', $this->mysql('hasil_radiologi')->where('no_rawat', $this->revertNorawat($id))->oneArray());
    ...
    public function getCatatan($id)
    {
      ...
      $vedika_feedback = $this->mysql('mlite_vedika_feedback')->where('nosep', $id)->asc('id')->toArray();
    ...
    public function getDownloadPDF($id)
    {
      ...
      $bridging_sep = $this->mysql('bridging_sep')->where('no_rawat', $this->revertNorawat($id))->oneArray();
    ...
    {
        $row = $this->mysql('prosedur_pasien')->join('icd9', 'icd9.kode = prosedur_pasien.kode')->where('prosedur_pasien.no_rawat', $no_rawat)->where('prosedur_pasien.prioritas', 1)->where('prosedur_pasien.status', $status_lanjut)->oneArray();
        return $row[$field];
    }
