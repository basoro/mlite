jQuery().ready(function () {
    var var_tbl_jns_perawatan_lab = $('#tbl_jns_perawatan_lab').DataTable({
        'processing': true,
        'serverSide': true,
        'serverMethod': 'post',
        'dom': 'Bfrtip',
        ...
        "bInfo" : false,
        "ajax": {
            "url": "{?=url(['jns_perawatan_lab','data'])?}",
            "dataType": "json",
            "type": "POST",
            "data": function (data) {

                // Read values
                var search_field_jns_perawatan_lab = $('#search_field_jns_perawatan_lab').val();
                var search_text_jns_perawatan_lab = $('#search_text_jns_perawatan_lab').val();
                
                data.search_field_jns_perawatan_lab = search_field_jns_perawatan_lab;
                data.search_text_jns_perawatan_lab = search_text_jns_perawatan_lab;
                
            }
        },
        "fnDrawCallback": function () {
            $('#more_data_jns_perawatan_lab').on('click', function(e) {
                e.preventDefault();
                var clientX = e.originalEvent.clientX;
                var clientY = e.originalEvent.clientY;
                $('#tbl_jns_perawatan_lab tr').contextMenu({x: clientX, y: clientY});
            });          
        },        
        "columns": [
            { 'data': 'kd_jenis_prw' },
    ...
    $.contextMenu({
        selector: '#tbl_jns_perawatan_lab tr', 
        trigger: 'right',
        callback: function(key, options) {
          var rowData = var_tbl_jns_perawatan_lab.rows({ selected: true }).data()[0];
          if (rowData != null) {
            var kd_jenis_prw = rowData['kd_jenis_prw'];
            switch (key) {
                case 'detail' :
                    OpenModal(mlite.url + '/jns_perawatan_lab/detail/' + kd_jenis_prw + '?t=' + mlite.token);
                break;
                default :
                break
            } 
    ...
    $("form[name='form_jns_perawatan_lab']").validate({
        rules: {
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                url: "{?=url(['jns_perawatan_lab','aksi'])?}",
                method: "POST",
                ...
                success: function (data) {
                    ...
                    if (typeact == "add") {
                        if(data.status === 'success') {
                            ...
                            $("#modal_jns_perawatan_lab").modal('hide');
                        } else {
                    ...
                    var_tbl_jns_perawatan_lab.draw();
                }
            })
        }
    });
    ...
    // ==============================================================
    $('#filter_search_jns_perawatan_lab').click(function () {
        var_tbl_jns_perawatan_lab.draw();
    });
    ...
    $("#edit_data_jns_perawatan_lab").click(function () {
        var rowData = var_tbl_jns_perawatan_lab.rows({ selected: true }).data()[0];
        if (rowData != null) {
            ...
            $('#modal-title').text("Edit Data Jns Perawatan Lab");
            $("#modal_jns_perawatan_lab").modal('show');
        }
        else {
    ...
    // ==============================================================
    jQuery("#hapus_data_jns_perawatan_lab").click(function () {
        var rowData = var_tbl_jns_perawatan_lab.rows({ selected: true }).data()[0];
    ...
    // ==============================================================
    jQuery("#tambah_data_jns_perawatan_lab").click(function () {

        $('#kd_jenis_prw').val('{?=$this->core->setKodeJnsPerawatanLab()?}');
        $('#nm_perawatan').val('');
        $('#bagian_rs').val('0');
        $('#bhp').val('0');
        $('#tarif_perujuk').val('0');
        ...
        $('#modal-title').text("Tambah Data Jns Perawatan Lab");
        $("#modal_jns_perawatan_lab").modal('show');
    });
    ...
    // ===========================================
    $("#lihat_data_jns_perawatan_lab").click(function () {
        ...
        $.ajax({
            ...
            data: {
                typeact: 'lihat', 
                search_field_jns_perawatan_lab: search_field_jns_perawatan_lab, 
                search_text_jns_perawatan_lab: search_text_jns_perawatan_lab
            },
            dataType: 'json',
            success: function (res) {
                var eTable = "<div class='table-responsive'><table id='tbl_lihat_jns_perawatan_lab' class='table display dataTable' style='width:100%'><thead><th>Kd Jenis Prw</th><th>Nm Perawatan</th><th>Bagian Rs</th><th>Bhp</th><th>Tarif Perujuk</th><th>Tarif Tindakan Dokter</th><th>Tarif Tindakan Petugas</th><th>Kso</th><th>Menejemen</th><th>Total Byr</th><th>Kd Pj</th><th>Status</th><th>Kelas</th><th>Kategori</th></thead>";
                ...
                $('#forTable_jns_perawatan_lab').html(eTable);
            }
        });

        $('#modal-title').text("Lihat Data");
        $("#modal_lihat_jns_perawatan_lab").modal('show');
    });
    ...
    $("#export_pdf").click(function () {
        ...
        doc.setFontSize(14);
        doc.text("Tabel Data Jns Perawatan Lab", 20, 95, null, null, null);
        const totalPagesExp = "{total_pages_count_string}";        
        doc.autoTable({
            html: '#tbl_lihat_jns_perawatan_lab',
            startY: 105,
            margin: {
                left: 20, 
                right: 20
        ...
        // doc.save('table_data_jns_perawatan_lab.pdf');
    ...
    $("#export_xlsx").click(function () {
        let tbl1 = document.getElementById("tbl_lihat_jns_perawatan_lab");
        let worksheet_tmp1 = XLSX.utils.table_to_sheet(tbl1);
        let a = XLSX.utils.sheet_to_json(worksheet_tmp1, { header: 1 });
        let worksheet1 = XLSX.utils.json_to_sheet(a, { skipHeader: true });
        const new_workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(new_workbook, worksheet1, "Data jns_perawatan_lab");
        XLSX.writeFile(new_workbook, 'tmp_file.xls');
    })

    $("#view_chart").click(function () {
        window.open(mlite.url + '/jns_perawatan_lab/chart?t=' + mlite.token, '_blank',"toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no,modal=yes");  
    }) 
