...
$("#index").on('click', '#bukaform', function(){
  ...
  $("#form").show().load(baseURL + '/api/notifikasiform?t=' + mlite.token);
...
$("#form").on("click", "#simpan", function(event){
  ...
  var judul = $('input:text[name=judul]').val();
  ...
  var url = baseURL + '/api/notifikasisave?t=' + mlite.token;
  ...
  $.post(url,{
    ...
    no_rkm_medis: no_rkm_medis
      ...
      $("#display").show().load(baseURL + '/api/notifikasidisplay?t=' + mlite.token);
      $("#form").hide();
      $("#tutupform").val("Buka Form");
      $("#tutupform").attr("id", "bukaform");
...
$("#display").on("click", ".edit", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url    = baseURL + '/api/notifikasiform?t=' + mlite.token;
...
$("#form").on("click","#hapus", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url = baseURL + '/api/notifikasihapus?t=' + mlite.token;
  var id = $('input:text[name=id]').val();
  ...
  bootbox.confirm("Apakah Anda yakin ingin menghapus data ini?", function(result){
    ...
    if (result){
      ...
      $.post(url, {
        ...
        $("#tutupform").val("Buka Form");
        $("#tutupform").attr("id", "bukaform");
        $("#display").load(baseURL + '/api/notifikasidisplay?t=' + mlite.token);
...
$('input:text[name=cari]').on('input',function(e){
  var baseURL = mlite.url + '/' + mlite.admin;
  var url    = baseURL + '/api/notifikasidisplay?t=' + mlite.token;
...
function bersih(){
  ...
  $('input:text[name=judul]').val("");
  $('input:text[name=tanggal]').val("");
