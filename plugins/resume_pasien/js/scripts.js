jQuery().ready(function () {
    var var_tbl_resume_pasien = $('#tbl_resume_pasien').DataTable({
        'processing': true,
        'serverSide': true,
        'serverMethod': 'post',
        'dom': 'Bfrtip',
        ...
        "bInfo" : false,
        "ajax": {
            "url": "{?=url(['resume_pasien','data'])?}",
            "dataType": "json",
            "type": "POST",
            "data": function (data) {

                // Read values
                var search_field_resume_pasien = $('#search_field_resume_pasien').val();
                var search_text_resume_pasien = $('#search_text_resume_pasien').val();
                
                data.search_field_resume_pasien = search_field_resume_pasien;
                data.search_text_resume_pasien = search_text_resume_pasien;
                
            }
        },
        "fnDrawCallback": function () {
            $('#more_data_resume_pasien').on('click', function(e) {
                e.preventDefault();
                var clientX = e.originalEvent.clientX;
                var clientY = e.originalEvent.clientY;
                $('#tbl_resume_pasien tr').contextMenu({x: clientX, y: clientY});
            });          
        }, 
        "columns": [
{ 'data': 'no_rawat' },
...
{ 'targets': 25}
    ...
    $.contextMenu({
        selector: '#tbl_resume_pasien tr', 
        trigger: 'right',
        callback: function(key, options) {
          var rowData = var_tbl_resume_pasien.rows({ selected: true }).data()[0];
          if (rowData != null) {
var no_rawat = rowData['no_rawat'];
            switch (key) {
                case 'detail' :
                    OpenModal(mlite.url + '/resume_pasien/detail/' + no_rawat + '?t=' + mlite.token);
                break;
                default :
                break
            } 
    ...
    $("form[name='form_resume_pasien']").validate({
        rules: {
no_rawat: 'required',
kd_dokter: 'required',
...
kondisi_pulang:'Kondisi Pulang tidak boleh kosong!',
...
formData.append('typeact', typeact); // tambahan
                ...
                success: function (data) {
                    ...
                    if (typeact == "add") {
                        if(data.status === 'success') {
                            ...
                            $("#modal_resume_pasien").modal('hide');
                        } else {
    ...
    // ==============================================================
    $('#filter_search_resume_pasien').click(function () {
        var_tbl_resume_pasien.draw();
    });
    ...
    $("#edit_data_resume_pasien").click(function () {
        var rowData = var_tbl_resume_pasien.rows({ selected: true }).data()[0];
        if (rowData != null) {
...
var kondisi_pulang = rowData['kondisi_pulang'];
...
$('#obat_pulang').val(obat_pulang);
            ...
            $('#modal-title').text("Edit Data Resume Pasien");
            $("#modal_resume_pasien").modal('show');
        }
        else {
    ...
    // ==============================================================
    jQuery("#hapus_data_resume_pasien").click(function () {
        var rowData = var_tbl_resume_pasien.rows({ selected: true }).data()[0];
...
var no_rawat = rowData['no_rawat'];
    ...
    // ==============================================================
    jQuery("#tambah_data_resume_pasien").click(function () {
...
$('#obat_pulang').val('');
        ...
        $('#modal-title').text("Tambah Data Resume Pasien");
        $("#modal_resume_pasien").modal('show');
    });
    ...
    // ===========================================
    $("#lihat_data_resume_pasien").click(function () {
        ...
        $.ajax({
            ...
            data: {
                typeact: 'lihat', 
                search_field_resume_pasien: search_field_resume_pasien, 
                search_text_resume_pasien: search_text_resume_pasien
            },
            dataType: 'json',
            success: function (res) {
                var eTable = "<div class='table-responsive'><table id='tbl_lihat_resume_pasien' class='table display dataTable' style='width:100%'><thead><th>No Rawat</th><th>Kd Dokter</th><th>Keluhan Utama</th><th>Jalannya Penyakit</th><th>Pemeriksaan Penunjang</th><th>Hasil Laborat</th><th>Diagnosa Utama</th><th>Kd Diagnosa Utama</th><th>Diagnosa Sekunder</th><th>Kd Diagnosa Sekunder</th><th>Diagnosa Sekunder2</th><th>Kd Diagnosa Sekunder2</th><th>Diagnosa Sekunder3</th><th>Kd Diagnosa Sekunder3</th><th>Diagnosa Sekunder4</th><th>Kd Diagnosa Sekunder4</th><th>Prosedur Utama</th><th>Kd Prosedur Utama</th><th>Prosedur Sekunder</th><th>Kd Prosedur Sekunder</th><th>Prosedur Sekunder2</th><th>Kd Prosedur Sekunder2</th><th>Prosedur Sekunder3</th><th>Kd Prosedur Sekunder3</th><th>Kondisi Pulang</th><th>Obat Pulang</th></thead>";
...
eTable += '<td>' + res[i]['kondisi_pulang'] + '</td>';
eTable += '<td>' + res[i]['obat_pulang'] + '</td>';
                    eTable += "</tr>";
                }
                eTable += "</tbody></table></div>";
                $('#forTable_resume_pasien').html(eTable);
            }
        });

        $('#modal-title').text("Lihat Data");
        $("#modal_lihat_resume_pasien").modal('show');
    });
    ...
    $("#export_pdf").click(function () {
        ...
        doc.setFontSize(14);
        doc.text("Tabel Data Resume Pasien", 20, 95, null, null, null);
        const totalPagesExp = "{total_pages_count_string}";        
        doc.autoTable({
            html: '#tbl_lihat_resume_pasien',
            startY: 105,
            margin: {
                left: 20, 
                right: 20
        ...
        // doc.save('table_data_resume_pasien.pdf');
    ...
    $("#export_xlsx").click(function () {
        let tbl1 = document.getElementById("tbl_lihat_resume_pasien");
        let worksheet_tmp1 = XLSX.utils.table_to_sheet(tbl1);
        let a = XLSX.utils.sheet_to_json(worksheet_tmp1, { header: 1 });
        let worksheet1 = XLSX.utils.json_to_sheet(a, { skipHeader: true });
        const new_workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(new_workbook, worksheet1, "Data resume_pasien");
        XLSX.writeFile(new_workbook, 'tmp_file.xls');
    })

    $("#view_chart").click(function () {
        window.open(mlite.url + '/resume_pasien/chart?t=' + mlite.token, '_blank',"toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no,modal=yes");  
    })   
