...
class Admin extends AdminModule
{
    ...
    public function postData(){
        ...
        if($search_text_kelurahan != ''){
            $searchQuery .= " and (".$search_field_kelurahan." like '%".$search_text_kelurahan."%' ) ";
        }

        ## Total number of records without filtering
        $sel = $this->db()->pdo()->prepare("select count(*) as allcount from kelurahan");
        $sel->execute();
        $records = $sel->fetch();
        $totalRecords = $records['allcount'];

        ## Total number of records with filtering
        $sel = $this->db()->pdo()->prepare("select count(*) as allcount from kelurahan WHERE 1 ".$searchQuery);
        $sel->execute();
        $records = $sel->fetch();
        $totalRecordwithFilter = $records['allcount'];

        ## Fetch records
        $sel = $this->db()->pdo()->prepare("select * from kelurahan WHERE 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row1.",".$rowperpage);
        $sel->execute();
...
$nm_kel = $_POST['nm_kel'];
            ...
            $kelurahan_add = $this->db()->pdo()->prepare('INSERT INTO kelurahan VALUES (?, ?)');
            $kelurahan_add->execute([$kd_kel, $nm_kel]);
...
$nm_kel = $_POST['nm_kel'];
        ...
        // BUANG FIELD PERTAMA
            ...
            $kelurahan_edit = $this->db()->pdo()->prepare("UPDATE kelurahan SET kd_kel=?, nm_kel=? WHERE kd_kel=?");
            $kelurahan_edit->execute([$kd_kel, $nm_kel,$kd_kel]);
        
        }

        if ($act=="del") {
            $kd_kel= $_POST['kd_kel'];
            $check_db = $this->db()->pdo()->prepare("DELETE FROM kelurahan WHERE kd_kel='$kd_kel'");
        ...
        if ($act=="lihat") {
            ...
            $user_lihat = $this->db()->pdo()->prepare("SELECT * from kelurahan WHERE 1 ".$searchQuery);
            $user_lihat->execute();
...
'nm_kel'=>$row['nm_kel']
    ...
    {
        $detail = $this->db('kelurahan')->where('kd_kel', $kd_kel)->toArray();
    ...
    public function getCss()
    {
        ...
        echo $this->draw(MODULES.'/kelurahan/css/admin/styles.css');
        exit();
    }
    ...
    public function getJavascript()
    {
        ...
        echo $this->draw(MODULES.'/kelurahan/js/admin/scripts.js', ['settings' => $settings]);
        exit();
    }
    ...
    private function _addHeaderFiles()
    {
        ...
        $this->core->addCSS(url([ADMIN, 'kelurahan', 'css']));
        $this->core->addJS(url([ADMIN, 'kelurahan', 'javascript']), 'footer');
    }
