<?php
namespace Plugins\Resume_Pasien_Ranap;
...
class Admin extends AdminModule
{
    ...
    public function getManage()
    {
        ...
        $disabled_menu = $this->core->loadDisabledMenu('resume_pasien_ranap'); 
    ...
    public function postData()
    {
        ...
        ## Custom Field value
        $search_field_resume_pasien_ranap= isset_or($_POST['search_field_resume_pasien_ranap']);
        $search_text_resume_pasien_ranap = isset_or($_POST['search_text_resume_pasien_ranap']);

        if ($search_text_resume_pasien_ranap != '') {
          $where[$search_field_resume_pasien_ranap.'[~]'] = $search_text_resume_pasien_ranap;
          $where = ["AND" => $where];
        } else {
          $where = [];
        }
        ...
        $totalRecords = $this->core->db->count('resume_pasien_ranap', '*');

        ## Total number of records with filtering
        $totalRecordwithFilter = $this->core->db->count('resume_pasien_ranap', '*', $where);

        ## Fetch records
        $where['ORDER'] = [$columnName => strtoupper($columnSortOrder)];
        $where['LIMIT'] = [$row1, $rowperpage];
        $result = $this->core->db->select('resume_pasien_ranap', '*', $where);
...
'tindakan_dan_operasi'=>$row['tindakan_dan_operasi'],
'obat_di_rs'=>$row['obat_di_rs'],
...
'alergi'=>$row['alergi'],
'diet'=>$row['diet'],
'lab_belum'=>$row['lab_belum'],
'edukasi'=>$row['edukasi'],
'cara_keluar'=>$row['cara_keluar'],
'ket_keluar'=>$row['ket_keluar'],
'keadaan'=>$row['keadaan'],
'ket_keadaan'=>$row['ket_keadaan'],
'dilanjutkan'=>$row['dilanjutkan'],
'ket_dilanjutkan'=>$row['ket_dilanjutkan'],
'kontrol'=>$row['kontrol'],
'obat_pulang'=>$row['obat_pulang']
        ...
        if($this->settings('settings', 'logquery') == true) {
          $this->core->LogQuery('resume_pasien_ranap => postData');
        }
    ...
    public function postAksi()
    {
        ...
        if ($act=='add') {
            ...
            if($this->core->loadDisabledMenu('resume_pasien_ranap')['create'] == 'true') {
...
$ket_dilanjutkan = $_POST['ket_dilanjutkan'];
$kontrol = $_POST['kontrol'];
$obat_pulang = $_POST['obat_pulang'];

            
            $result = $this->core->db->insert('resume_pasien_ranap', [
'no_rawat'=>$no_rawat, 'kd_dokter'=>$kd_dokter, 'diagnosa_awal'=>$diagnosa_awal, 'alasan'=>$alasan, 'keluhan_utama'=>$keluhan_utama, 'pemeriksaan_fisik'=>$pemeriksaan_fisik, 'jalannya_penyakit'=>$jalannya_penyakit, 'pemeriksaan_penunjang'=>$pemeriksaan_penunjang, 'hasil_laborat'=>$hasil_laborat, 'tindakan_dan_operasi'=>$tindakan_dan_operasi, 'obat_di_rs'=>$obat_di_rs, 'diagnosa_utama'=>$diagnosa_utama, 'kd_diagnosa_utama'=>$kd_diagnosa_utama, 'diagnosa_sekunder'=>$diagnosa_sekunder, 'kd_diagnosa_sekunder'=>$kd_diagnosa_sekunder, 'diagnosa_sekunder2'=>$diagnosa_sekunder2, 'kd_diagnosa_sekunder2'=>$kd_diagnosa_sekunder2, 'diagnosa_sekunder3'=>$diagnosa_sekunder3, 'kd_diagnosa_sekunder3'=>$kd_diagnosa_sekunder3, 'diagnosa_sekunder4'=>$diagnosa_sekunder4, 'kd_diagnosa_sekunder4'=>$kd_diagnosa_sekunder4, 'prosedur_utama'=>$prosedur_utama, 'kd_prosedur_utama'=>$kd_prosedur_utama, 'prosedur_sekunder'=>$prosedur_sekunder, 'kd_prosedur_sekunder'=>$kd_prosedur_sekunder, 'prosedur_sekunder2'=>$prosedur_sekunder2, 'kd_prosedur_sekunder2'=>$kd_prosedur_sekunder2, 'prosedur_sekunder3'=>$prosedur_sekunder3, 'kd_prosedur_sekunder3'=>$kd_prosedur_sekunder3, 'alergi'=>$alergi, 'diet'=>$diet, 'lab_belum'=>$lab_belum, 'edukasi'=>$edukasi, 'cara_keluar'=>$cara_keluar, 'ket_keluar'=>$ket_keluar, 'keadaan'=>$keadaan, 'ket_keadaan'=>$ket_keadaan, 'dilanjutkan'=>$dilanjutkan, 'ket_dilanjutkan'=>$ket_dilanjutkan, 'kontrol'=>$kontrol, 'obat_pulang'=>$obat_pulang
            ]);
            ...
            if($this->settings('settings', 'logquery') == true) {
              $this->core->LogQuery('resume_pasien_ranap => postAksi => add');
            }
        ...
        if ($act=="edit") {
            ...
            if($this->core->loadDisabledMenu('resume_pasien_ranap')['update'] == 'true') {
              http_response_code(403);
              $data = array(
                'code' => '403', 
                'status' => 'error', 
...
$obat_pulang = $_POST['obat_pulang'];
        ...
        // BUANG FIELD PERTAMA
            ...
            $result = $this->core->db->update('resume_pasien_ranap', [
'no_rawat'=>$no_rawat, 'kd_dokter'=>$kd_dokter, 'diagnosa_awal'=>$diagnosa_awal, 'alasan'=>$alasan, 'keluhan_utama'=>$keluhan_utama, 'pemeriksaan_fisik'=>$pemeriksaan_fisik, 'jalannya_penyakit'=>$jalannya_penyakit, 'pemeriksaan_penunjang'=>$pemeriksaan_penunjang, 'hasil_laborat'=>$hasil_laborat, 'tindakan_dan_operasi'=>$tindakan_dan_operasi, 'obat_di_rs'=>$obat_di_rs, 'diagnosa_utama'=>$diagnosa_utama, 'kd_diagnosa_utama'=>$kd_diagnosa_utama, 'diagnosa_sekunder'=>$diagnosa_sekunder, 'kd_diagnosa_sekunder'=>$kd_diagnosa_sekunder, 'diagnosa_sekunder2'=>$diagnosa_sekunder2, 'kd_diagnosa_sekunder2'=>$kd_diagnosa_sekunder2, 'diagnosa_sekunder3'=>$diagnosa_sekunder3, 'kd_diagnosa_sekunder3'=>$kd_diagnosa_sekunder3, 'diagnosa_sekunder4'=>$diagnosa_sekunder4, 'kd_diagnosa_sekunder4'=>$kd_diagnosa_sekunder4, 'prosedur_utama'=>$prosedur_utama, 'kd_prosedur_utama'=>$kd_prosedur_utama, 'prosedur_sekunder'=>$prosedur_sekunder, 'kd_prosedur_sekunder'=>$kd_prosedur_sekunder, 'prosedur_sekunder2'=>$prosedur_sekunder2, 'kd_prosedur_sekunder2'=>$kd_prosedur_sekunder2, 'prosedur_sekunder3'=>$prosedur_sekunder3, 'kd_prosedur_sekunder3'=>$kd_prosedur_sekunder3, 'alergi'=>$alergi, 'diet'=>$diet, 'lab_belum'=>$lab_belum, 'edukasi'=>$edukasi, 'cara_keluar'=>$cara_keluar, 'ket_keluar'=>$ket_keluar, 'keadaan'=>$keadaan, 'ket_keadaan'=>$ket_keadaan, 'dilanjutkan'=>$dilanjutkan, 'ket_dilanjutkan'=>$ket_dilanjutkan, 'kontrol'=>$kontrol, 'obat_pulang'=>$obat_pulang
            ], [
              'no_rawat'=>$no_rawat
            ]);
            ...
            if($this->settings('settings', 'logquery') == true) {
              $this->core->LogQuery('resume_pasien_ranap => postAksi => edit');
            }
        ...
        if ($act=="del") {
            ...
            if($this->core->loadDisabledMenu('resume_pasien_ranap')['delete'] == 'true') {
              http_response_code(403);
              $data = array(
                'code' => '403', 
                'status' => 'error', 
            ...
            $no_rawat= $_POST['no_rawat'];
            $result = $this->core->db->delete('resume_pasien_ranap', [
              'AND' => [
                'no_rawat'=>$no_rawat
              ]
            ]);
            ...
            if($this->settings('settings', 'logquery') == true) {
              $this->core->LogQuery('resume_pasien_ranap => postAksi => del');
            }
        ...
        if ($act=="lihat") {
            ...
            if($this->core->loadDisabledMenu('resume_pasien_ranap')['read'] == 'true') {
              http_response_code(403);
              $data = array(
                'code' => '403', 
                'status' => 'error', 
            ...
            $search_field_resume_pasien_ranap= $_POST['search_field_resume_pasien_ranap'];
            $search_text_resume_pasien_ranap = $_POST['search_text_resume_pasien_ranap'];
...
'obat_pulang'=>$row['obat_pulang']
            ...
            if($this->settings('settings', 'logquery') == true) {
              $this->core->LogQuery('resume_pasien_ranap => postAksi => lihat');
            }
    ...
    public function getRead($no_rawat)
    {
        ...
        $result =  $this->core->db->get('resume_pasien_ranap', '*', ['no_rawat' => $no_rawat]);
        ...
        if($this->settings('settings', 'logquery') == true) {
          $this->core->LogQuery('resume_pasien_ranap => getRead');
        }
    ...
    public function getDetail($no_rawat)
    {
        ...
        if($this->settings('settings', 'logquery') == true) {
          $this->core->LogQuery('resume_pasien_ranap => getDetail');
        }
    ...
    public function getChart($type = '', $column = '')
    {
      ...
      $labels = $this->core->db->select('resume_pasien_ranap', 'kd_dokter', ['GROUP' => 'kd_dokter']);
      $datasets = $this->core->db->select('resume_pasien_ranap', ['count' => \Medoo\Medoo::raw('COUNT(<kd_dokter>)')], ['GROUP' => 'kd_dokter']);

      if(isset_or($column)) {
        $labels = $this->core->db->select('resume_pasien_ranap', ''.$column.'', ['GROUP' => ''.$column.'']);
        $datasets = $this->core->db->select('resume_pasien_ranap', ['count' => \Medoo\Medoo::raw('COUNT(<'.$column.'>)')], ['GROUP' => ''.$column.'']);          
      }

      $database = DBNAME;
      $nama_table = 'resume_pasien_ranap';
      ...
      if($this->settings('settings', 'logquery') == true) {
        $this->core->LogQuery('resume_pasien_ranap => getChart');
      }
    ...
    public function getCss()
    {
        ...
        echo $this->draw(MODULES.'/resume_pasien_ranap/css/styles.css');
        exit();
    }
    ...
    public function getJavascript()
    {
        ...
        $settings = $this->settings('settings');
        echo $this->draw(MODULES.'/resume_pasien_ranap/js/scripts.js', ['settings' => $settings, 'disabled_menu' => $this->core->loadDisabledMenu('resume_pasien_ranap')]);
        exit();
    }
    ...
    private function _addHeaderFiles()
    {
        ...
        $this->core->addCSS(url([ 'resume_pasien_ranap', 'css']));
        $this->core->addJS(url([ 'resume_pasien_ranap', 'javascript']), 'footer');
    }
