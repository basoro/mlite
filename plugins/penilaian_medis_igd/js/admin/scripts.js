jQuery().ready(function () {
    var var_tbl_mlite_penilaian_medis_igd = $('#tbl_mlite_penilaian_medis_igd').DataTable({
        'processing': true,
        'serverSide': true,
        'serverMethod': 'post',
        'dom': 'Bfrtip',
        ...
        "bInfo" : false,
        "ajax": {
            "url": "{?=url([ADMIN,'penilaian_medis_igd','data'])?}",
            "dataType": "json",
            "type": "POST",
            "data": function (data) {

                // Read values
                var search_field_mlite_penilaian_medis_igd = $('#search_field_mlite_penilaian_medis_igd').val();
                var search_text_mlite_penilaian_medis_igd = $('#search_text_mlite_penilaian_medis_igd').val();
                
                data.search_field_mlite_penilaian_medis_igd = search_field_mlite_penilaian_medis_igd;
                data.search_text_mlite_penilaian_medis_igd = search_text_mlite_penilaian_medis_igd;
...
{ 'targets': 35}
    ...
    $("form[name='form_mlite_penilaian_medis_igd']").validate({
        rules: {
no_rawat: 'required',
tanggal: 'required',
kd_dokter: 'required',
...
var tata= $('#tata').val();
 ...
 formData.append('typeact', typeact); // tambahan
            ...
            $.ajax({
                url: "{?=url([ADMIN,'penilaian_medis_igd','aksi'])?}",
                method: "POST",
    ...
    // ==============================================================
    $('#search_text_mlite_penilaian_medis_igd').keyup(function () {
        var_tbl_mlite_penilaian_medis_igd.draw();
    });
    // ==============================================================
    // CLICK TANDA X DI INPUT SEARCH
    // ==============================================================
    $("#searchclear_mlite_penilaian_medis_igd").click(function () {
        $("#search_text_mlite_penilaian_medis_igd").val("");
        var_tbl_mlite_penilaian_medis_igd.draw();
    });
    ...
    $("#edit_data_mlite_penilaian_medis_igd").click(function () {
        var rowData = var_tbl_mlite_penilaian_medis_igd.rows({ selected: true }).data()[0];
        if (rowData != null) {
...
$('#tata').val(tata);
            ...
            $('#modal-title').text("Edit Data Penilaian Medis IGD");
            $("#modal_mlite_penilaian_medis_igd").modal();
        }
        else {
    ...
    // ==============================================================
    jQuery("#hapus_data_mlite_penilaian_medis_igd").click(function () {
        var rowData = var_tbl_mlite_penilaian_medis_igd.rows({ selected: true }).data()[0];
...
var no_rawat = rowData['no_rawat'];
    ...
    if(window.location.search.indexOf('no_rawat') !== -1) { 
        $('#search_text_mlite_penilaian_medis_igd').val(searchParams.get('no_rawat'));
        var_tbl_mlite_penilaian_medis_igd.draw();
    ...
    jQuery("#tambah_data_mlite_penilaian_medis_igd").click(function () {
...
$('#tata').val('');
        ...
        $('#modal-title').text("Tambah Data Penilaian Medis IGD");
        $("#modal_mlite_penilaian_medis_igd").modal();
    });
    ...
    // ===========================================
    $("#lihat_data_mlite_penilaian_medis_igd").click(function () {
...
eTable += '<td>' + res[i]['tata'] + '</td>';
                ...
                $('#forTable_mlite_penilaian_medis_igd').html(eTable);
            }
        });

        $('#modal-title').text("Lihat Data");
        $("#modal_lihat_mlite_penilaian_medis_igd").modal();
    });

    // ==============================================================
    // TOMBOL DETAIL mlite_penilaian_medis_igd DI CLICK
    // ==============================================================
    jQuery("#lihat_detail_mlite_penilaian_medis_igd").click(function (event) {
        ...
        if (rowData) {
            ...
            var loadURL =  baseURL + '/penilaian_medis_igd/detail/' + no_rawat.replace(/\//g,'') + '?t=' + mlite.token;
        
            var modal = $('#modal_detail_mlite_penilaian_medis_igd');
            var modalContent = $('#modal_detail_mlite_penilaian_medis_igd .modal-content');
    ...
    jQuery("#lihat_detail_mlite_penilaian_medis_igd2").click(function (event) {
        ...
        if (rowData) {
            ...
            {if: $this->core->ActiveModule('jasper')}
                var loadURL =  baseURL + '/jasper/awalmedisigd/' + no_rawat.replace(/\//g,'') + '?t=' + mlite.token;
                $("#modal_detail_mlite_penilaian_medis_igd").modal('show').html('<div style="text-align:center;margin:20px auto;width:90%;height:95%;"><iframe src="' + loadURL + '" frameborder="no" width="100%" height="100%"></iframe></div>');
            {else}
    ...
    $("#export_pdf").click(function () {
        ...
        doc.setFontSize(14);
        doc.text("Tabel Data Mlite Penilaian Medis Igd", 20, 95, null, null, null);
        const totalPagesExp = "{total_pages_count_string}";        
        doc.autoTable({
            html: '#tbl_lihat_mlite_penilaian_medis_igd',
            startY: 105,
            margin: {
                left: 20, 
                right: 20
        ...
        // doc.save('table_data_mlite_penilaian_medis_igd.pdf')
    ...
    $("#export_xlsx").click(function () {
        let tbl1 = document.getElementById("tbl_lihat_mlite_penilaian_medis_igd");
        let worksheet_tmp1 = XLSX.utils.table_to_sheet(tbl1);
        let a = XLSX.utils.sheet_to_json(worksheet_tmp1, { header: 1 });
        let worksheet1 = XLSX.utils.json_to_sheet(a, { skipHeader: true });
        const new_workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(new_workbook, worksheet1, "Data mlite_penilaian_medis_igd");
