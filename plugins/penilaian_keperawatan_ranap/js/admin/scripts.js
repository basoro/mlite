...
eTable += '<td>' + res[i]['kd_dokter'] + '</td>';
    ...
    jQuery("#lihat_detail_mlite_penilaian_awal_keperawatan_ranap2").click(function (event) {
        ...
        if (rowData) {
            ...
            {if: $this->core->ActiveModule('jasper')}
                var loadURL =  baseURL + '/jasper/awalkeperawatanranap/' + no_rawat.replace(/\//g,'') + '?t=' + mlite.token;
                $("#modal_detail_mlite_penilaian_awal_keperawatan_ranap").modal('show').html('<div style="text-align:center;margin:20px auto;width:90%;height:95%;"><iframe src="' + loadURL + '" frameborder="no" width="100%" height="100%"></iframe></div>');
            {else}
