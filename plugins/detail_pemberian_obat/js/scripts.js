jQuery().ready(function () {
    var var_tbl_detail_pemberian_obat = $('#tbl_detail_pemberian_obat').DataTable({
        ...
        "bInfo" : false,
        "ajax": {
            "url": "{?=url(['detail_pemberian_obat','data'])?}",
            "dataType": "json",
            "type": "POST",
            "data": function (data) {
        ...
        "fnDrawCallback": function () {
            $('#more_data_detail_pemberian_obat').on('click', function(e) {
                e.preventDefault();
                var clientX = e.originalEvent.clientX;
                var clientY = e.originalEvent.clientY;
                $('#tbl_detail_pemberian_obat tr').contextMenu({x: clientX, y: clientY});
            });          
        }, 
        "columns": [
{ 'data': 'tgl_perawatan' },
...
{ 'targets': 13}
    ...
    $.contextMenu({
        selector: '#tbl_detail_pemberian_obat tr', 
        trigger: 'right',
...
var tgl_perawatan = rowData['tgl_perawatan'];
            switch (key) {
                case 'detail' :
                    OpenModal(mlite.url + '/detail_pemberian_obat/detail/' + tgl_perawatan + '?t=' + mlite.token);
                break;
                default :
                break
            } 
...
biaya_obat:'Biaya Obat tidak boleh kosong!',
...
formData.append('typeact', typeact); // tambahan
            ...
            $.ajax({
                url: "{?=url(['detail_pemberian_obat','aksi'])?}",
                method: "POST",
                ...
                success: function (data) {
                    ...
                    if (typeact == "add") {
                        if(data.status === 'success') {
                            ...
                            $("#modal_detail_pemberian_obat").modal('hide');
                        } else {
    ...
    // ==============================================================
    $('#filter_search_detail_pemberian_obat').click(function () {
        var_tbl_detail_pemberian_obat.draw();
    });
...
$('#no_faktur').val(no_faktur);
            ...
            $("#modal_detail_pemberian_obat").modal('show');
        }
        else {
...
eTable += '<td>' + res[i]['no_faktur'] + '</td>';
        ...
        $("#modal_lihat_detail_pemberian_obat").modal('show');
    });
    ...
    $("#view_chart").click(function () {
        window.open(mlite.url + '/detail_pemberian_obat/chart?t=' + mlite.token, '_blank',"toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no,modal=yes");  
    })   
