...
class Admin extends AdminModule
{
    ...
    private function testDecrypt($encrypted, $originalPlain)
    {
        try {
            // Decrypt untuk test
            $decrypted = $this->decryptDataMR($encrypted, $this->consid, $this->secretkey, $this->koders);
            
            // Bandingkan dengan original
            return $decrypted === $originalPlain;
        } catch (\Exception $e) {
            error_log("Decrypt test failed: " . $e->getMessage());
            return false;
        }
    }

    // Method decrypt untuk testing (balikan dari makeDataMR)
    public function decryptDataMR($encrypted, $consid, $secretkey, $koders)
    {
        // Decode base64
        ...
        if (!$plain) {
            throw new \Exception('Gzip decompress failed');
        }
